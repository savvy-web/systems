...
The `e2e/*` workspace area holds private, never-published harness packages that exercise the repo's build and release tooling the way a real consumer does: through the BUILT `dist/dev` artifact, against an isolated fixture repo or a scratch project outside the workspace. Its original reason to exist is `catalog:`/`workspace:` specifier resolution, which roots at `process.cwd()`, so the only faithful and hermetic way to test it is to run the real tool with `cwd` pointed at a fixture that owns its own `pnpm-workspace.yaml`. Two later harnesses cover the package graph itself: `@e2e/workspace` asserts the declared layering, and `@e2e/silk` proves the carrier bins from a packed install.
...
`e2e/*` is a top-level glob in `pnpm-workspace.yaml` matching four harness packages: `@e2e/bundler`, `@e2e/pnpm-plugin-silk`, `@e2e/silk` and `@e2e/workspace`. All are `private: true`, and in the [package layering](../workspace/package-layering.md) they form the harness band, which may depend on anything. Each declares the package(s) under test as `workspace:*` devDependencies, so on install they link the real built `dist/dev` artifact and consume it exactly as a downstream repo would — through the published entry points, never the source tree. `@e2e/workspace` is the one exception to the built-artifact rule: it reads manifests off disk and depends on no app package.
...
All four harness packages are green in the normal test gate. `@e2e/bundler` spawns the built `@savvy-web/bundler` front door (and its raw-tsdown escape hatch) inside subprocess fixtures; `@e2e/pnpm-plugin-silk` imports the built `pnpmfile.mjs` and spawns the built `savvy` binary; `@e2e/silk` packs the six app packages and installs silk's tarball into scratch projects under pnpm and npm; `@e2e/workspace` walks the live dependency graph. None has a runtime of its own — everything under `e2e/` is test code plus fixture data.
...
**Tier 2b — `@e2e/pnpm-plugin-silk` (real `CatalogResolver` via the spawned `savvy` binary).** `regen-catalog.e2e.test.ts` closes the gap the other tiers leave: `Changesets.DepsRegen` is unit-tested only against a mocked resolver. The test copies the `regen-catalog` fixture into a `mkdtempSync` dir OUTSIDE the repo, `git init`s it as the base commit, edits the working tree to add `catalog:`/`workspace:`/dev dependencies, then spawns the built `@savvy-web/cli` binary (`savvy changeset deps regen`) with `cwd` set to that dir. It asserts the emitted changeset carries concrete resolved versions rather than protocol strings and no `devDependency` rows, that `savvy changeset check` passes and that the file lints clean through the same `@savvy-web/silk/changesets/markdownlint` custom-rule path the pre-commit hook uses. The silk rules resolve through the root `devDependencies` links; since the root stopped depending on `@savvy-web/cli` directly (silk carries the bins), the harness declares its own explicit `@savvy-web/cli: workspace:*` devDependency so the package resolves by name.

**Tier 3 — `@e2e/silk` (the carrier bins from a packed install OUTSIDE the workspace).** `packed-install.e2e.test.ts` is the proof behind the carrier decision in [package-layering.md](../workspace/package-layering.md#the-carrier-decision). `packAll` runs `pnpm pack --config.ignore-scripts=true --pack-destination <tmp>/tarballs` in each of the six app packages' SOURCE directories: pnpm honours `publishConfig.directory`, so the tarball's contents ARE `dist/dev/pkg`, while the manifest goes through pnpm's publish rewrite (`workspace:*` → exact sibling version, `catalog:` → catalogued range). Packing with cwd inside `dist/dev/pkg` instead fails with `ERR_PNPM_CANNOT_RESOLVE_WORKSPACE_PROTOCOL` (the dir is not a workspace member and `devManifest: "preserve"` leaves the protocols in), and without `ignore-scripts` pack replays each package's `prepare` build (~15 s each versus ~0.3 s). A test asserts every tarball manifest is free of `workspace:`/`catalog:`, is `private: false`, and that silk's declares `bin: { savvy, savvy-mcp }` and pins its companions to the exact versions packed beside it.

For each of `pnpm` and `npm` (a manager missing from `PATH` is `it.skip`ped by name), a `realpathSync`-ed `mkdtemp` scratch project gets `devDependencies: { "@savvy-web/silk": "file:<tarball>" }` and nothing else, with the five companions steered to their tarballs via overrides — npm's manifest `overrides`, and for **pnpm 11** a `pnpm-workspace.yaml` carrying only `overrides:`, because the `pnpm` field in `package.json` is no longer read. The pnpm install passes `--config.strict-dep-builds=false`: pnpm 11 exits non-zero on `ERR_PNPM_IGNORED_BUILDS` (esbuild's optional postinstall) even though the install completed, and that flag is a supply-chain posture, not hoisting, so it is passed on the CLI rather than written into the scratch config. The test then asserts `node_modules/.bin/{savvy,savvy-mcp}` exist and are executable, that no `.npmrc`, hoist pattern, `configDependencies` or `@savvy-web/pnpm-plugin-silk` is present, that the scratch lockfile records each of the six packages as `file:…/<tarball>` (the realpath step exists because macOS's `/var` → `/private/var` symlink otherwise makes pnpm record a different relative path), and runs both bins exactly as the in-repo test does: `savvy --version` exits 0 with a semver on stdout; `savvy-mcp` fed one `initialize` line then EOF answers with `"jsonrpc":"2.0"` and `serverInfo` on stdout, stderr exactly `""`, exit 0. Under pnpm the `.bin` entries are asserted to be silk's shims; under npm's flat layout they may resolve to the front ends' own mirror bins, so the test only asserts the linked file lives inside the scratch project's `node_modules/@savvy-web/`. It needs the network for `effect` and the `@effected` kit. Bins are spawned through `runBin` in `helpers.ts` — `@effected/commands` `Run.collect` over `ChildProcess.make` with `SPAWN_ENV` — so this is the one harness on `@effect/vitest`. The in-repo half, the same two bins straight out of `packages/silk/dist/dev/pkg/bin`, is `packages/silk/__test__/e2e/bins.e2e.test.ts`.

**Tier 4 — `@e2e/workspace` (the package graph respects the declared layering).** `package-graph.e2e.test.ts` reads every `packages/*/package.json` and `e2e/*/package.json` off disk, extracts every `workspace:*` edge across `dependencies`, `devDependencies` and `peerDependencies`, and checks it against `layers.json` — the single source of truth for which package sits in which layer, which are tooling and which are harness. An edge may only go from a higher layer to a strictly lower one or into the tooling band; same-layer packages never reference each other; tooling never references an app layer; `@e2e/*` is unconstrained; the whole graph topologically sorts. Offenders print as `"<from> -> <to> (<field>)"`. Two hand-built fixture graphs (a sideways L3 edge, a three-node cycle) are the positive controls, and the live assertion requires a non-empty edge inventory so it cannot pass vacuously. The package has no fixtures, spawns nothing, and depends only on `@savvy-web/bundler` so its `tsconfig.json` `extends` resolves — a harness→tooling edge the rules ignore. The full rule set and what it protects is in [package-layering.md](../workspace/package-layering.md#the-dag-check).
...
- Every spawn passes `SPAWN_ENV` (see [Gotchas](#gotchas)); `@e2e/silk` builds its own copy because it does not import the bundler helpers.
- Tests run in the normal `pnpm test` gate via `AgentPlugin.discover()`, serialised per project — no separate vitest project, no separate CI job. `@e2e/silk` is the slow one (two package-manager installs with network); keeping it in its own project is what keeps it serialised.
- `@e2e/silk` packs from the SOURCE package dir with `ignore-scripts`, never from `dist/dev/pkg`, and never writes a hoist pattern, `.npmrc` or config dependency into the scratch project — the absence is the assertion.
...
**pnpm 11 reads overrides from `pnpm-workspace.yaml`, not `package.json`.** The `pnpm` manifest field is ignored with a warning (`The following keys were ignored: "pnpm.overrides"`), so a scratch project that needs `overrides` writes a `pnpm-workspace.yaml` holding nothing else. pnpm also writes `allowBuilds`/`minimumReleaseAgeExclude` into that file during install; the forbidden-key check in `@e2e/silk` is a substring match over `publicHoistPattern`/`hoistPattern`/`configDependencies`/`shamefully`, so those keys do not trip it. And `strictDepBuilds` makes pnpm 11 fail an install that merely ignored an optional build — pass `--config.strict-dep-builds=false` on the CLI.
...
The host repo cannot be a fixture for its own resolver tests, and it cannot be a consumer of its own published packages either — which is why the carrier bins are proven from a scratch project that knows nothing about this workspace. Once the catalogs that resolution reads are the host's own live config, the test asserts against a moving target and couples to the repo's config-dependency state — a coupling that broke the previous in-package integration tests when `@savvy-web/pnpm-plugin-silk` moved in-repo. Spawning the real built tool against a fixture that owns its workspace root makes the test both hermetic (resolution can only see fixture catalogs) and high-fidelity (it runs the published artifact through the entry point a consumer uses). The subprocess boundary is not incidental — it is what makes `process.cwd()`-based resolution testable without an API change in `@effected/workspaces`.
...
- `../workspace/package-layering.md` — the layering `@e2e/workspace` asserts and the carrier decision `@e2e/silk` proves.
- `../silk/architecture.md`, `../mcp/architecture.md` — the bins and the server the Tier 3 handshake exercises.
- `../testing/effect-vitest.md` — suite-wide test conventions; `@e2e/bundler`, `@e2e/pnpm-plugin-silk` and `@e2e/workspace` run no Effect in-process and stay on plain `vitest`, while `@e2e/silk` drives its bins through `@effected/commands` on `@effect/vitest`.
