...
SP1 implemented, plus Track A (API Extractor meta generation, the `src/meta/` module), Track C (the `src/targets/` module — `publishConfig.targets` derivation into byte-variant groups, plus the name-aware TargetGroup plumbing that lets the build loop carry a per-group renamed manifest), Track D (the `src/jsx/` module — tsconfig→rolldown JSX mapping threaded through the dts tsconfig and the build loop), Track B (the `src/exe/` module — SEA executable compilation over `@tsdown/exe`), the §8 config-validation service (`src/config-validation/`), M1 (configurable esm/cjs output format, no new module — it threads `BuildFormat`/`dual`/`cjsDefault` through the existing build loop and manifest transform), the M2 self-host and the M3 bundled-dts two-pass build (no new module — it splits the build loop into a JS pass and a dts pass via `deriveTargetGroupOptions`/`deriveDtsPassOptions`). **This package now builds itself** via an escape-hatch `savvy.build.ts` (it dropped the rslib devDeps + `rslib.config.ts`, added `tsx`, and keeps a synced local `ecma.json` tsconfig base — see `../bundler/architecture.md`). Runtime `dependencies` are `workspaces-effect` (`^1.2.0`, for `CatalogResolver`), `@effect/platform-node` (provides `NodeContext` at the resolution boundary), `sort-package-json`, `std-env`, `picocolors`, `json-schema-effect`, plus the meta deps `@microsoft/api-extractor`, `@microsoft/tsdoc`, `@microsoft/tsdoc-config` and `deep-equal`; `effect` is a `catalog:silkPeers` peer. `tsdown`/`rolldown` are devDependencies — types only. (The `@tsdown/exe` runtime dependency lives on the *bundler*, not here — tsdown lazily imports it; see `../bundler/architecture.md`.)

Out of SP1/Tracks A/B/C/D/M1/M2/M3: the SP2 preflight findings the reporter will later carry, per-variant meta (meta is still emitted once into the canonical group) and **Track E (full publishability)**, which is not done. M2 self-hosting IS done — this package and the bundler self-build, and the four leaf libraries build through the bundler front door; only `@savvy-web/cli`, `@savvy-web/mcp` and `@savvy-web/silk` remain rslib-built. The error set, formatter set, service set, the `resolveTargets` validation cases and the `ConfigValidator` rule set are discoverable in source — this doc documents the topology, not the enumerations.
...
- **JS pass:** `unbundle: true`, `dts: false`, default `clean: true`. Per-module `.js` (plus `.cjs` when dual). Runs the `emitManifest` plugin and the `public/` copy exactly once.
