...
Runtime `dependencies`: `workspaces-effect` (`CatalogResolver`), `@effect/platform-node` (`NodeContext` at the resolution boundary), `typescript` (the tsconfig-resolver uses the TS compiler API at runtime; externalized in this package's own `savvy.build.ts`), the meta deps `@microsoft/api-extractor`/`@microsoft/tsdoc`/`@microsoft/tsdoc-config`, plus `sort-package-json`/`std-env`/`picocolors`/`json-schema-effect`/`deep-equal`. `effect` is a `catalog:silkPeers` peer; `tsdown`/`rolldown` are devDependencies (types only). The `@tsdown/exe` runtime dependency lives on the *bundler*, which lazily imports it (see `../bundler/architecture.md`).

Not yet done: per-variant meta (meta is emitted once into the canonical group) and **Track E (full publishability)**, the release-action consumer of the targets binding. The error set, formatter set, service set, the `resolveTargets` validation cases and the `ConfigValidator` rule set are discoverable in source — this doc documents the topology, not the enumerations.
