...
- **Seam 2 — emitted-file byte metrics via `buildMetricsPlugin` (`src/report/metrics-plugin.ts`).** A rolldown plugin whose `writeBundle` records each emitted chunk/asset's byte size from the in-memory content (gzip computed only when `verbose`, since gzip is the expensive part); `onLog` is defensive rolldown-diagnostic capture. It fires for the JS pass AND the `emitDtsOnly` dts pass, so both passes' files land in the report.
