...
- **Per-group diagnostic dedup.** Mirroring `seenPaths`, `recordWarning`/`recordError` dedup per group via a `seenDiagnostics` set keyed on source+level+text+file+line+column, so a warning that fires in more than one pass (e.g. a dual-format entry's JS and dts passes) is reported once rather than per pass.
...
- **`BuildReport` Schema (`src/report/schema.ts`):** per package → per TargetGroup `{ id, entries, passes, warnings[], errors[], timings }`. `passes` is a `PassReport[]` (`{ id: "js"|"dts"|"loose"|"exe"|"meta", files: EmittedFile[], ms }`) where `EmittedFile` is `{ path, bytes, gzip? }`, replacing the former flat `emittedFiles: string[]`. `warnings`/`errors` are now `DiagnosticEntry[]` (`{ source: "tsdown"|"rolldown"|"api-extractor", level: "warn"|"error", text, file?, line?, column? }`) rather than plain strings, so a formatter can render `file:line` and a CI annotation can emit `file=`/`line=`. The schema is fed by the `BuildCollector` (see [The build collector and capture seams](#the-build-collector-and-capture-seams)); see the file for the exact `Schema.Struct`s. A SchemaStore-compatible JSON Schema is emitted via `json-schema-effect` (`src/report/schema-export.ts`) so the structured `json`/`markdown` output validates and editors get autocomplete.
- **Quiet-by-default terminal, verbose table.** The `terminal` formatter is quiet by default: a bold package name, one line per target group (`<glyph> <id>  <N> files · <time>`), warnings/errors inline with `file:line` and a final aggregate line. `FormatterContext.verbose` (threaded by `pipeline.ts` from the render options) switches it to a per-pass file table with byte sizes (plus gzip). `markdown`/`ci-annotations` consume the structured `DiagnosticEntry`.

## The meta-generation pipeline

`src/meta/` holds **all** the meta behavior the bundler wires; the bundler owns no meta logic of its own (see `../bundler/architecture.md`). Meta is the API Extractor `.api.json` model the bundler emits into other packages' `localPaths` (for downstream API-doc consumers like mcp) and into each prod group's `meta/` release-asset bundle. The bundler runs `generateMeta` during `--target prod`, over each group's emitted prod `.d.ts` and against its catalog-resolved prod `package.json` — API Extractor never re-runs the tsdown bundle, but meta reads PROD output, not dev (the dev manifest keeps unresolved `catalog:`/`workspace:*` specifiers; see `../bundler/architecture.md`).

The module is a set of separately-tested units; each file is the source of truth for its own mechanics:

- **`generateMeta` (`generate.ts`)** — the orchestrator. Writes `tsdoc.json`, runs the extractor once per entry into `outMetaDir`, takes the single model or `mergeApiModels` when more than one entry, writes the **"virtual TS env" trio** into `outMetaDir` — `<unscoped>.api.json` + the FINAL transformed `package.json` (read from the built `pkg/` dir, not the source manifest) + a PORTABLE derived `tsconfig.json` (from `resolvePortableTsconfig`) — copies that trio into each `localPaths` dir, and removes the per-entry `*.entry.api.json` intermediates so they never leak into the bundle. The bundle `package.json` is now READ-then-WRITTEN (not a verbatim `copyFileSync`) so an optional `manifestTransform(pkg)` can rewrite it before it lands in `outMetaDir` and the `localPaths` copies — the seam the bundler uses for the optimistic next-version rewrite. The api-extractor `tsdoc-metadata.json` is written into the built `pkg/` dir (a published-package artifact TSDoc tooling reads from the package root), **not** into the meta bundle.
...
- **All meta behavior lives here.** `generateMeta` and its units (including `rewriteMetaVersions` and `resolveNextVersions`) own the API Extractor pipeline and the optimistic next-version rewrite; the bundler only wires it. Meta runs over already-emitted `.d.ts` (the bundler drives it over the PROD output during `--target prod`), never re-bundling. `resolveNextVersions` is a deliberate copy of silk-effects' release-plan slice — silk-effects is downstream of this toolchain and importing it would cycle.
