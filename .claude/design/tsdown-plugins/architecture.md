...
- **`BuildReport` Schema:** per package → per TargetGroup `{ entries, emittedFiles, timings, warnings[], errors[] }`. A SchemaStore-compatible JSON Schema is emitted via `json-schema-effect` (`src/report/schema-export.ts`) so the structured `json`/`markdown` output validates and editors get autocomplete.
