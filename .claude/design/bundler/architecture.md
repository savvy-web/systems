...
dist/
  dev/                  # the dev TargetGroup (registry-less, local-link only)
    ...
    issues.json         # structured build diagnostics (always written; tsdown/rolldown only — no meta pass)
  prod/
    targets.json        # the TargetResolution binding for the release action
    issues.json         # structured build diagnostics (always written; includes the prod-only ae-*/tsdoc- meta pass)
