...
- **No boilerplate.** `build()` derives `cwd` from `dirname(process.argv[1])` (the equivalent of `import.meta.dirname` for a plain-script file) and `argv` from `process.argv.slice(2)`, so the build file reads no process metadata by hand. The old `export default config` + `if (import.meta.main) { await runBuild(config, { cwd: import.meta.dirname, argv: ... }) }` guard is gone — nothing imports `savvy.build.ts` files, so the guard was protecting an import that never happens.
...
`savvy.build.ts` replaces rslib's inscrutable factory-notation `rslib.config.ts`. It is agent-legible and a reliable signal the silk plugin can detect.

## build, defineBuild and runBuild

`build(input?, overrides?)` (`src/run.ts`) is the front door for `savvy.build.ts` files: it calls `runBuild(defineBuild(input), { cwd: dirname(process.argv[1]), argv: process.argv.slice(2) })`. The `cwd` is the directory containing the build script — the faithful equivalent of the old `import.meta.dirname` — and `argv` is the raw CLI argument list. An optional partial `RunOptions` passed as `overrides` takes precedence over the derived values for cases where the defaults need overriding. Escape hatches and tests call `defineBuild`/`runBuild` directly.

The two underlying functions are deliberately separated so the config surface stays pure and the orchestration stays injectable.

- **`defineBuild(input)`** (`src/config.ts`) normalizes and validates the config into a `BuildConfig`, applying defaults (`devManifest: "preserve"`, `transform: defaultManifestTransform`, `minify: false`). It does **not** itself run the build — that is `runBuild`'s job. The optional `format?: ReadonlyArray<BuildFormat>` is the live dual-format field. A pre-existing dead `formats: ReadonlyArray<"esm">` field still sits beside it but is not consumed by the build — `format` is the one `runBuild` reads. See `BuildConfigInput` in `src/config.ts` for the full input shape.
- **`runBuild(config, options)`** (`src/run.ts`) is the orchestrator: parse argv (including `--verbose`) → read `package.json` at cwd → resolve effective jsx (explicit override ?? `readTsconfigJsx` inference) → write the resolved dts tsconfig → derive entries (`packageJsonEntries`) → **run `ConfigValidator.validate(...)` first to fast-fail bad config** → instantiate a `BuildCollector` → branch on `--target`: `meta` and `exe` short-circuit before the main build; otherwise resolve any per-entry `overrides` into base+override entry partitions (computing the `dualExports` Set) and `normalizeLooseFiles(config.looseFiles)` into descriptors → derive the build groups (a single `dev` group, or all prod groups from `publishConfig.targets`) → call `buildTargetGroups` (the collector, `config.format`, the posture knobs, `overrides`/`dualExports` and the normalized `looseFiles` threaded in via conditional spread; on `--target prod` also `emitDeclarations: true`, activating the third per-module declarations pass that emits `dist/prod/<id>/declarations/` — the input `runMetaPass` feeds as `aeInputDir` into each `generateMeta` call for the two-input split) → on `--target prod` write the `targets.json` binding, call `runMetaPass` (the tsdown-plugins helper that loops `generateMeta` per group, routing diagnostics into the collector via `onMessage`) then `removeDeclarationMaps` on each prod group's `pkg/` → render ONE unified log from `collector.snapshot(packageName)` via `renderReport`/`ReportPipelineLive` → on `dev`/`prod` write the `dist/<target>/issues.json` artifact via the injectable `writeIssues` hook (defaults to `writeIssuesArtifact` from tsdown-plugins, the structured-diagnostics counterpart of the rendered log — see `../tsdown-plugins/architecture.md`). Every IO dependency is injectable on `RunOptions` so the orchestration is unit-testable without touching disk or spawning tsdown.
- **The issues write happens on every terminal path, and stamps the outcome.** `writeIssuesBestEffort(err?)` runs on the success path AND in the `catch` before the rethrow — a failed build is exactly when an agent wants to read why. Its definition sits immediately after the `BuildCollector`, and the outer `try` opens there rather than at the build call, so setup and `ConfigValidator` failures are covered too. On both paths the artifact is persisted BEFORE the log renders, and the `catch`'s render is wrapped in its own swallow, so neither a renderer failure nor a missing renderer (a crash before `renderAndWrite` is assigned) can lose the artifact or displace the original error. The `--target meta` and `--target exe` early returns stay inside the `try` and deliberately do not use a `finally`, which would start rendering on paths that have no build log to render. Because of that, the write passes `buildOk: err === undefined` plus, on failure, a `failure: { name?, message }` reduced by the local `describeFailure` (an `Error`'s `name`, else a tagged object's `_tag`/`name`, else `String(err)`). Without that stamp a crash that produced zero diagnostics writes an artifact byte-identical to a clean gate, so **readers gate on `buildOk`, not on `errors.length`** — the contract lives in `../tsdown-plugins/architecture.md`. The write stays best-effort (a read-only fs or missing parent is swallowed) so it can never compound or mask the real build outcome.
...
- **`defineBuild({ format })`** takes an optional `format?: ReadonlyArray<BuildFormat>` (`"esm" | "cjs"`, re-exported from `@savvy-web/tsdown-plugins`). `runBuild` forwards `config.format` into the `buildTargetGroups` call (conditional spread, mirroring `define`); `tsdown-plugins` defaults it to `["esm"]` when unset.
...
- **The meta entry is repointed for the subdir.** `applySubdirMetaEntries` (now a `@savvy-web/tsdown-plugins` helper called inside `runMetaPass`, no longer a `run.ts` helper) overwrites the override's meta dts basename to `<outSubdir>/index` (keyed by the stable flattened entry name) so `generateMeta` reads the bundled dts at `pkg/<outSubdir>/index.d.ts` rather than a root-level `<flatName>.d.ts`. This is what feeds a `./runtime` export's API model into the merged `.api.json`. `runBuild` keeps a `validateSubdirOverrides` fast-fail (mirroring the override-loop checks) so a malformed `outSubdir` override is rejected on every target path before `runMetaPass` runs.
...
- **Declaration source-maps are stripped from prod.** `runBuild` calls `removeDeclarationMaps` on each prod group's `pkg/` AFTER meta generation has consumed the `.d.ts.map`/`.d.cts.map` files (the dts tsconfig emits them; API Extractor reads them; they are dead weight that leaks local paths in the published tarball). Dev keeps them so `--target meta` can read them. The two self-hosting builders call `removeDeclarationMaps` directly in their escape-hatch `savvy.build.ts` — AFTER their `runMetaPass` call, since meta consumes the maps (see [Self-hosting](#self-hosting-the-bootstrap-ladder)).
...
dist/
  dev/                  # the dev TargetGroup (registry-less, local-link only)
    ...
    issues.json         # structured build diagnostics (always written, atomically; read buildOk first; tsdown/rolldown only — no meta pass)
  prod/
    targets.json        # the TargetResolution binding for the release action
    issues.json         # structured build diagnostics (always written, atomically; read buildOk first; includes the prod-only ae-*/tsdoc- meta pass)
    npm/                # one folder per distinct byte-variant group; npm is the single-target default
      ...
      declarations/     # per-module unbundled dts tree (prod-only, never published) — API Extractor Run B input
...
- **Clean turbo caching:** `build:dev → dist/dev/**`, `build:prod → dist/prod/**` are disjoint cache lanes. `build:dev`'s `outputs` were widened to add `dist/dev/issues.json` (its `dist/dev/pkg/**` did not cover the sibling artifact); `build:prod`'s `dist/prod/**` already caches `dist/prod/issues.json`.
...
The bundler is pure wiring over `@savvy-web/tsdown-plugins`' meta surface (all the behavior lives there — see `../tsdown-plugins/architecture.md`); the bundler decides *when* and *where*. **The meta orchestration itself is now `runMetaPass`** — a single tsdown-plugins helper that derives export paths, filters `bin/` entries, resolves the optimistic next-versions and loops `generateMeta` once per prod group. The front door (`run.ts`) deleted its inline meta block and calls `runMetaPass`; both self-hosting escape hatches call the SAME helper (the meta-pass unification — the bundler now decides even less, reinforcing the "all meta lives in tsdown-plugins" invariant). **Meta generation runs inside `--target prod`**, reading each prod group's own emitted `.d.ts` and `package.json` — it still does **not** re-run the tsdown bundle, but it is no longer a standalone decoupled target.
...
- **`deriveExportPaths`** (now a `@savvy-web/tsdown-plugins` export, moved out of `run.ts`) recovers export keys from the package `exports` map to drive the per-entry extraction. `runMetaPass` calls it internally; `run.ts` still imports it to compute the per-entry export-key map for the override-partition `dualExports` derivation.
- **`bin/` entries are excluded from the dts basenames meta reads.** `runMetaPass` collects the `dtsBasenames` it feeds to `generateMeta` from the build entries, skipping any `bin/`-prefixed entry — the dts pass no longer emits a `.d.ts` for a side-effect-only bin (see `../tsdown-plugins/architecture.md`), so meta must not try to read a declaration that was never written.
...
- **One consumer, one resolution:** the resolved `jsx` flows only into the generated dts tsconfig (`writeResolvedTsconfig({ jsx, jsxImportSource })`), which both the JS and dts passes consume via `dts: { tsconfig }`, so emit over `.tsx` sees the right runtime. It is **not** also forwarded into `buildTargetGroups`/rolldown `inputOptions` — tsdown rejects a top-level `jsx` input-options key, so that forward was redundant (the tsconfig already carries it) and only emitted a per-pass "Invalid input options" warning (issue #170, removed).
...
`runBuild` delegates the per-group build loop to `buildTargetGroups` (`tsdown-plugins/src/build/build-target-groups.ts`), which calls `tsdown.build()` with config-file loading bypassed (`config: false`) and inline options derived from `deriveTargetGroupOptions`. The loop runs **two logical passes per TargetGroup** to the same outDir — a JS pass (`unbundle: true`, `dts: false`) and a bundled-dts pass (`unbundle: false`, `dts: { emitDtsOnly: true }`, `clean: false`) — so the build emits per-module JS plus a single rolled-up `.d.ts` per public entry. The bundled-dts pass now runs one single-entry `build()` per entry so its `.d.ts`/`.d.cts` and the `.api.json` derived from them are byte-deterministic across rebuilds (#185/#156); the per-entry mechanics, the scoped `stableTypeOrdering` emit tsconfig and the re-export-stub optimization live in `../tsdown-plugins/architecture.md`. On `--target prod` a third pass (`unbundle: true`, `dts: { emitDtsOnly: true }`) emits a per-module declaration tree into `dist/prod/<id>/declarations/` for API Extractor's diagnostics-only run (see [Meta generation wiring](#meta-generation-wiring) and `../tsdown-plugins/architecture.md`). The two-pass mechanics and rationale live in `../tsdown-plugins/architecture.md`. Fixed choices that cross this boundary:

- `unbundle: true` (rolldown `preserveModules`) on the JS pass replaces rslib's `disableSharedChunks` — one-to-one source→output, no shared cross-entry runtime chunk, which sidesteps the multi-entry ESM `__webpack_require__` collision rslib worked around. The dts pass uses `unbundle: false` so declarations roll up. The one exception: a `bundleNodeModules` build's JS pass also runs `unbundle: false` so the output is a single npm-pack-safe self-contained file — see [Bundling-posture knobs](#bundling-posture-knobs) and `../tsdown-plugins/architecture.md`.
...
- **`plugins` forwards custom rolldown plugins to EVERY tsdown run.** `defineBuild({ plugins })` takes a `ReadonlyArray<Plugin>` (the rolldown `Plugin` type, imported from `rolldown` and re-exported from `@savvy-web/bundler`). `runBuild` conditional-spreads `config.plugins` into the `buildTargetGroups` call under the internal name `extraPlugins` (public `plugins` → internal `extraPlugins`); tsdown-plugins already spreads `extraPlugins` into the `plugins: [...]` of all four `build(...)` calls — the JS pass, the bundled-dts pass, the per-module declarations pass and each looseFiles pass — so a single plugin list runs across the whole build. Plugins sit after the builder's internal interop plugins and before its metrics instrumentation. The driver is build-time codegen / virtual modules: a consumer's looseFiles `pnpmfile.ts` can `import` a virtual module that a config-dependency plugin (e.g. `pnpm-config-builder`'s `PnpmConfigPlugin`) serves via `resolveId`/`load`, because the user plugin runs on the looseFiles pass where the pnpmfile is bundled. Forwarding needs no change to `@savvy-web/tsdown-plugins` — the `extraPlugins` plumbing predates the bundler surfacing it. **Because the bundler's own public `BuildConfigInput.plugins` field types as `ReadonlyArray<Plugin>` and `index.ts` re-exports `Plugin`, the bundler declares `rolldown` as a direct dependency** so the (huge) `Plugin` type tree stays an external `import("rolldown")` reference in the bundler's emitted `.d.ts` rather than being inlined — inlining drags in rolldown-internal symbols the entry point does not export, which API Extractor flags as `ae-forgotten-export` (a hard error under CI). This applies only to the bundler's self-build, whose public surface exposes `Plugin`; a downstream package that merely passes `plugins` to `defineBuild` does not expose `Plugin` in its own declarations, so its dts is unaffected.
...
- **The bundler owns no build behavior.** Every behavior is a helper in `@savvy-web/tsdown-plugins`; the bundler only wires them. Anything the front door does, a hand-written `tsdown.config.ts` can do by importing the same helper. This includes meta: `runMetaPass`/`generateMeta`/`resolveNextVersions`/`rewriteMetaVersions`/`deriveExportPaths`/`applySubdirMetaEntries` all live in tsdown-plugins; the front door and both self-hosting escape hatches call the SAME `runMetaPass`. The bundler only decides when (inside `--target prod`) and where (per group into `dist/prod/<group>/meta`, the canonical group also into `localPaths`).
