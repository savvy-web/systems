...
- **The Record-map form is now in use across the repo.** Every in-repo package declares `publishConfig.targets` as the Record map (single-key `{ npm: true }` in the common case), so `--target prod` derives its prod group(s) for real — Track C is live, not dormant. The legacy-array guard remains in the reader as a tolerance fallback (an array-valued `targets`, the old rslib-builder shape, reads as `undefined` and falls back to the single-`npm` default), but no in-repo package relies on it anymore. Only the Record map activates a non-default group.
...
- **Track C is live; the legacy-array path is only a tolerance fallback.** Every in-repo package declares the Record-map `publishConfig.targets`, so `--target prod` derives prod groups from it for real. An array-valued `targets` (the old rslib-builder shape) still reads as `undefined` and falls back to the single-`npm` default, but nothing in the repo depends on that path anymore.
