...
The silk plugin spawns the shared `savvy-mcp` server (tools-only — see `../mcp/architecture.md`) and orients the agent toward its tools. The combined `hooks/session-start/orientation.sh` hook (see [Hook merge](#hook-merge)) emits a single always-on `<silk_capabilities>` block: a "use the tools, don't shell out or answer from memory" directive, the full ten-tool MCP index, the three-agent index (changeset-manager, turborepo, tsdoctor), the eight-skill index with auto-load annotations, the Biome division of labor (LSP → `biome_check` → Bash escape hatch), and a compact active-hooks map. The block sells the whole capability surface but stays index-shaped: per-capability detail lives at point-of-use — the path-triggered `changeset-style`/`build`/`tsdoc` skills, the on-demand `turbo`/`repos` skills, and the PreToolUse nudges. The hook has no matcher, so it fires on every start including resume/compact.
...
- `session-start/orientation.sh` — **no matcher**, so it fires on every start including resume and compact. It is the env **producer**: it detects the package manager, writes the session vars and dedup-appends them to `$CLAUDE_ENV_FILE` (see the namespace note below). It also emits the always-on `<silk_capabilities>` block — the MCP tool, agent, and skill indexes plus the Biome division of labor and the active-hooks map (see [MCP tool orientation](#mcp-tool-orientation)).
