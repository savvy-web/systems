...
- **The Biome LSP — automatic diagnostics.** An `lspServers.biome` block in `.claude-plugin/plugin.json` (same shape as the `mcpServers` block) launches Biome's language server so lint/format diagnostics on the file the agent just edited land in context with zero prompting. The `command` is `sh` running `bin/biome-lsp.sh`; `transport` and `diagnostics` stay default (stdio / on). `extensionToLanguage` covers js/mjs/cjs, jsx, ts/mts/cts, tsx, json/jsonc, css and graphql/gql — deliberately **no** html (the suite has none). The LSP is diagnostics-only: it sees only open/edited files (not a whole-repo scan) and **cannot apply fixes** — fixing is the MCP tool's or Bash's job.
...
- **`biome_check` MCP tool — structured execution with fixes.** The shared `savvy-mcp` server exposes a `biome_check` tool that runs Biome over a path and returns parsed diagnostics, supporting read-only check (`mode`) plus safe (`write`) and unsafe (`unsafe`) fixes. This is the "I want a structured result or want to apply fixes" channel the LSP cannot provide. See `../mcp/architecture.md` for the tool (it is the first mutating savvy-mcp tool, an intentional read-only-convention exception).
- **The `biome-prefer-mcp` PreToolUse hook — nudge, never block.** `hooks/pre-tool-use/biome-prefer-mcp.sh` (a third `matcher: "Bash"` PreToolUse entry in `hooks.json`, alongside `changeset-push-guard.sh` and `commit-bash.sh`) detects when the agent runs Biome through Bash — either directly (`biome`, `*/biome`, `pnpm exec biome`, `npx biome`, word-boundary matched) or indirectly via a package-manager/turbo script whose nearest `package.json` body mentions `biome` — and emits a one-time-per-session `additionalContext` nudge toward `biome_check`. It is **purely additive**: it emits **no** `permissionDecision`, so the command always proceeds and Bash Biome stays a valid escape hatch. The once-per-session marker keys on the envelope `session_id` under `~/.claude/session-env/`, falling back to a fixed `_no-session` key when no session id is present so the once invariant still holds. It fails open without `jq` and is best-effort on the indirect case (only the nearest root `package.json` is consulted, so workspace-scoped script invocations may be missed). The MCP proxy and the LSP both run Biome in their own processes (not the Bash tool), so neither self-triggers the hook.
- **The `<biome_capability>` orientation block.** `hooks/session-start/orientation.sh` carries a moderate-tier block (not `EXTREMELY_IMPORTANT`) teaching the division of labor: the LSP shows problems automatically but cannot fix (do not shell `biome check <file>` just to inspect a file you edited); `biome_check` checks or fixes any path with a structured result (`mode`/`write`/`unsafe`); Bash Biome is the raw escape hatch. It records the `biome-prefer-mcp` hook so the agent knows a Bash Biome call draws a one-time nudge, not a block.
