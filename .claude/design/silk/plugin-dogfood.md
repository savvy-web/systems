...
Multi-loop routing is the monitor's job, because a mailbox is per counterpart while a journal is per loop. `scanJournals()` takes a journal's counterpart from the snapshot's `counterpart.id` when present and falls back to the filename stem, deriving the loop id from the remainder; `diagnose()` then groups journals by counterpart and tries to PIN each mail file to one of them by its `loop:` key. Pinning is what decides routing, and only a `loop:` key naming a journal that exists pins anything: a file with no `loop:` key, or one naming a journal that is absent, is pinned to nothing, and a default journal for that counterpart does not claim it — the default survives only as the attribution hint behind the display label. Unpinned mail belongs to the counterpart rather than to one of its loops, so it fans out over ALL of that counterpart's journals: judged against the LOWEST watermark among them (new if new to at least one) and its notified state recorded under EVERY one of their ids. Both halves are load-bearing in exactly the migration shape above — a bare `<counterpart-id>.jsonl` kept beside a new `<counterpart-id>.<loop>.jsonl`, which every existing loop passes through, and where a default journal DOES exist. Narrowing to that default recorded the file only under the bare id, so the turn-flip suppression, which looks mail up by `journal.id`, missed it and re-fired for the loop-scoped journal (#339). And the tempting `?? 0` watermark for mail no single journal owns is the exact defect the `loop-started` fallback exists to prevent: it makes every archived file newer than the watermark and replays a closed collaboration as unread on the first tick after a second loop opens (#344).
