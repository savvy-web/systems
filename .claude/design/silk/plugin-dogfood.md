...
Multi-loop routing is the monitor's job, because a mailbox is per counterpart while a journal is per loop. `scanJournals()` takes a journal's counterpart from the snapshot's `counterpart.id` when present and falls back to the filename stem, deriving the loop id from the remainder; `diagnose()` then groups journals by counterpart and tries to PIN each mail file to one of them by its `loop:` key. Pinning is what decides routing, and only a `loop:` key naming a journal that exists pins anything: a file with no `loop:` key, or one naming a journal that is absent, is pinned to nothing, and a default journal for that counterpart does not claim it — the default survives only as the attribution hint behind the display label. Unpinned mail belongs to the counterpart rather than to one of its loops, so it fans out over that counterpart's LIVE journals — those whose last snapshot is not a terminal `unlinked` — judged against the LOWEST watermark among them (new if new to at least one) and its notified state recorded under EVERY one of their ids on every tick it stands above that mark, not only on the tick that announces it. Each clause is a repaired defect. Keying off the pin rather than the default matters in exactly the migration shape — a bare `<counterpart-id>.jsonl` kept beside a new `<counterpart-id>.<loop>.jsonl`, which every existing loop passes through and where a default DOES exist — because narrowing to it recorded the file only under the bare id, so the turn-flip suppression, which looks mail up by `journal.id`, missed it and re-fired for the loop-scoped journal (#339). Recording on every qualifying tick, because a journal can appear AFTER a file was first surfaced — `--init` opening a second loop for mail just announced is the sequence this feature exists to enable — and starts with an empty notified set the correctly suppressed re-announce would never fill, leaving the new loop's first `ball: ours` append to fire a spurious turn alert (#339). And the live filter, because a journal persists after `--exit` with no archival ceremony and nothing can be new to a loop that is over: left in the set, a closed loop drags the bar back to its own `loop-started` and replays both loops' archives (#344). Once EVERY loop for a counterpart is closed the rule INVERTS: unpinned mail is judged against the HIGHEST watermark among those closed loops instead. Nothing any of them already processed can be new, but anything past all of them still is — and a fresh `request` on a finished collaboration is how a session learns the counterpart wants to reopen, which `--init` exists to be prompted into. Skipping the file swallowed that signal permanently: unannounced, it was never recorded either, so a later `loop-started` moved the mark past its mtime and it stayed invisible for good. The `unlinked` quiescence SKILL.md describes is scoped to the TURN alert, not to inbound mail — the two paths deliberately differ. For a single journal the inverted rule collapses to exactly the `lastMail.in` watermark the pre-loop-scoped monitor used, so it generalizes that behavior rather than changing it, and it keeps the bar off 0, which is #344 itself. A counterpart with NO journal at all is a separate case, keeping its existing everything-is-new behavior.
