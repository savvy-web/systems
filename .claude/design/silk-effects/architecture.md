...
- **`ReposLockdown`** (`src/repos/services/lockdown.ts`) is the boundary itself: `lock` chmods a vendored tree to files `0444` / directories `0555`, `unlock` restores `0644`/`0755`, and `withUnlocked(root, name, effect)` brackets a mutation between the two with `Effect.ensuring`, so an interrupt or a mid-flight git failure still re-locks. Failures surface as `ReposLockdownError { path, reason }`. `ReposLockdown.layer` needs only `FileSystem | Path`.
...
- **`sync`/`add`/`pin` re-lock explicitly after staging, on top of `withUnlocked`'s finalizer.** The finalizer re-locks through `Effect.ignore`, so a chmod failure there is swallowed; the explicit `lockdown.lock(root, name)` that follows is the call that surfaces a `ReposLockdownError` to the caller. It also covers the work that happens *outside* the unlocked window — `configStore.write` plus `git add` of the manifest and gitlink. The double-lock is deliberate and commented as such; do not "dedupe" it.

Consequence for consumers: `sync`, `add` and `pin` all widen their error channel with `ReposLockdownError`, which propagates into `reposCommand`'s annotated error channel in the CLI and into `repos_manage`'s in the MCP server. Reads (`status`, `repos_inspect`) work unchanged against a locked tree — reading never needs write permission — and `.repos/config.json` is host-repo content that is never locked.
