...
- **`ReposManager`** (`src/repos/services/manager.ts`) is the submodule plumbing, and now covers the whole vendored-repo lifecycle: `status`, `sync`, `add`, `pin`, `note`, `remove`, `rename` and `restore` — detailed in [Lifecycle operations](#lifecycle-operations-reposmanager) below. All git plumbing goes through `@effected/git`'s `Git`, whose typed failures are mapped onto this module's `GitSubmoduleError`. `ReposManager.layer` requires `ReposConfigStore | Git | FileSystem | Path | ReposLockdown`.
