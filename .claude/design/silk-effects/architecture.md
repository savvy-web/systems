...
src/
  ...
  services/             ← Context.Service services with Live layers
...
Each service is a `Context.Service` class with a companion `*Shape` interface — the shape is exported so consumers (and test doubles) can name it:
...
Live layers carry an explicit `Layer.Layer<Service, Error, Requirements>` annotation so a requirement added by accident shows up as a type error rather than silently widening the consumer graph:

```typescript
// Pure service (no dependencies)
export const ServiceLive: Layer.Layer<ServiceName> = Layer.succeed(ServiceName, { ... });

// Service with dependencies
export const ServiceLive: Layer.Layer<ServiceName, never, FileSystem.FileSystem> = Layer.effect(
  ServiceName,
  Effect.gen(function* () {
...
- `SilkPublishabilityDetectorLive` (override of `@effected/workspaces`' `PublishabilityDetector`)
- `PublishabilityDetectorAdaptiveLive` (also requires `ChangesetConfig`; its vanilla branch reads the
...
- `Changesets.DepsRegen` / `Changesets.DepsRegenLive`
  ...
  `ChangesetConfig`, `FileSystem` (`PointInTimeWorkspace` + `PublishabilityDetector` from `@effected/workspaces`;
...
- `Changesets.DepsRegenDefault` (batteries-included)
  Requires only the platform services — composes `PointInTimeWorkspaceLive`, `ConfigInspectorLive`
  and `PublishabilityDetectorAdaptiveLive` internally; needs a git-capable platform layer
