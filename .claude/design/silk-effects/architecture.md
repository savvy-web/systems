...
- **`ReposDrift`** (`src/repos/services/drift.ts`) is the read-only reconciler: `check(root)` compares the four authorities a vendored repo's state is spread across and reports every disagreement — see [Drift reconciliation](#drift-reconciliation-reposdrift). `ReposDrift.layer` requires `ReposConfigStore | Git | FileSystem | Path` — deliberately NOT `ReposLockdown`, because it never writes and runs unmodified against a locked tree.
...
Six decisions in this area are load-bearing:
...
`check(root)` reconciles four authorities — the manifest, `.gitmodules`, the worktree, and `git submodule status` — returning a `ReposDriftReport { drifts, clean }`. Each `RepoDrift` carries `name`, a `kind`, a human-readable `detail`, and for value mismatches the disagreeing `manifestValue`/`observedValue` pair. The eight `DriftKind`s: `urlMismatch`, `pathMismatch`, `unregisteredManifestEntry`, `orphanGitmodulesEntry`, `missingWorktree`, `checkoutDiverged`, `missingShallow`, `gitmodulesUnparsable`.

Three reconciliation decisions are load-bearing:
