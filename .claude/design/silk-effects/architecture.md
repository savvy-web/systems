...
CSH005's semantics are unified across both engines (#456/#457): a shared scanner, `scanDependencySection` in `src/changesets/utils/dependency-section.ts`, is consumed by BOTH the remark and markdownlint `dependency-table-format` rules, so the rule means the same thing everywhere — a valid 5-column table **anywhere** inside the `## Dependencies` section satisfies it, with prose allowed before and after the table, and a missing-table diagnostic anchors at the heading in both engines (previously the two rules disagreed on where a table could sit and where the diagnostic landed). `RULE_DOCS` (`src/changesets/constants.ts`) points at in-repo rule documentation, `packages/silk-effects/docs/rules/CSH001.md`–`CSH005.md` — the former home, the `savvy-web/changesets` repo, is archived.
...
- **File attribution has a fixed precedence, and a versioned root is always LAST.** `classifyOne` resolves a path to a package in order: (1) directory containment, deepest `workspaceDir` wins; (2) `additionalScopes`; (3) `versionFiles`; (4) a root-as-package fallback. A scope whose `workspaceDir` IS the project dir is deliberately held OUT of the containment pass and applied only at step 4, because every file in the repo is "inside" the root by definition — letting the root compete on containment made it win any path outside a sub-package directory and silently shadow the `additionalScopes` and `versionFiles` the config declared for that path. The last-resort placement is what lets a single-package repo whose only package IS the root attribute its files at all, without outranking a more specific claim. `checkConflicts` carves the root out of shadowing validation for the same reason: the root represents the whole repo, not a sub-package whose directory claims ownership. Step 4 still applies `isInside(projectDir, abs)` before returning the root — `resolve` accepts `../` and absolute inputs, and the root owning a path that is not in the repo would be a worse answer than owning nothing. A path that falls through all four steps is UNMAPPED, but may carry a machine-readable **`unmappedHint`** reason (#290, a new `ClassificationReason` union member): the pure `unmappedHint` helper matches the relative path against every scope's `versionFiles`/`additionalScopes` globs (the deleted-target shape branch diffs produce — the glob names the path but no file materializes behind it) and against a small static `TEMPLATE_MIRRORS` table (repo files kept in byte-lockstep with a template a package ships, today the markdownlint config mirroring `src/lint/cli/templates/markdownlint.gen.ts`). The hint is context, not attribution — `package` stays `null`, the hint just tells the agent WHY the path is probably already accounted for; membership in the mirror table is an `Object.hasOwn` check for the same prototype-pollution reason `ReleasePlanner`'s `changelogModules` uses one.
- **Worktree callers get per-call-root discovery** (#487). A long-lived host builds the discovery layer ONCE for its startup root, yet a caller in a git worktree of the same repository inspects a DIFFERENT `projectDir` — and an agent worktree (`.claude/worktrees/*`) is NESTED inside the primary checkout, so mixing primary workspace dirs with worktree file paths made the shadowing check see every worktree path as "inside" the primary root. `inspect` derives its per-call root from `WorkspaceDiscovery.listPackagesIn`: discovery re-READS beneath the directory passed to `inspect` (memoized per root), so a worktree answers from its OWN manifests — membership, names AND versions come from the actual per-call root, not an approximation from the layer-bound root's manifests — and `refreshIn` gives per-root cache invalidation.
...
- **The release-line renderer is AST-native: emit decisions are made on MDAST node types, never string prefixes.** `parseChangesetSections` (`src/changesets/utils/section-parser.ts`) exposes `contentNodes` — the MDAST slice under each heading — alongside the string `content` on `ParsedSection`, and `getReleaseLine` renders from the nodes (`renderSectionNode` in `src/changesets/changelog/getReleaseLine.ts`): a **paragraph** becomes a `-` bullet with continuation lines indented two spaces (the historical prose behavior); a **heading** is promoted one level, clamped to depths 4–6, so a changeset `###` sub-heading emits as `####` and can never collide with the changelog's depth-3 category headings that the version-block utilities and merge/reorder/dedup plugins treat as section boundaries; **everything else** (tables, code fences, blockquotes, lists) passes through verbatim as a block — never bullet-wrapped, never continuation-indented. Attribution attaches to the LAST paragraph across the entry's content nodes (or the last paragraph of the last list item), parsed to phrasing nodes and appended before rendering — never into a table cell or a heading; when no paragraph target exists it renders standalone. The golden corpus test `__test__/changesets/pipeline__corpus.test.ts` pins whole rendered entries end-to-end.
- **Contributor thanks aggregate into a `### Thanks` section pinned LAST, and `thanks: false` strips them.** `ContributorFootnotesPlugin` (`src/changesets/remark/plugins/contributor-footnotes.ts`) now lifts inline `Thanks @user!` attributions into a `### Thanks` heading section, which `ReorderSectionsPlugin` pins last in the version block (priority 1000). It is the one preset member that takes options: a new `thanks: boolean` changelog option (default `true`) on `ChangesetOptionsSchema` and `TransformOptions`, plumbed through `ChangelogTransformer` and `ReleasePlanner` (read from the `.changeset/config.json` changelog options tuple); `thanks: false` strips attribution entirely — no inline thanks, no Thanks section — while PR references are kept either way (provenance, not thanks; `getReleaseLine` honors the same flag when building the attribution suffix). `AggregateDependencyTablesPlugin` also unwraps legacy bullet-wrapped dependency tables and merges authored + synthesized tables into ONE table per version block, so historical CHANGELOG shapes converge on the current format instead of accumulating duplicates.
- **`Changesets.vanillaChangelogFunctions` re-exports the stock renderer.** `src/changesets/changelog/vanilla.ts` re-exports `@changesets/changelog-git` (a new runtime dependency, `^1.0.0`) for consumers who want the upstream renderer without depending on it directly (#413) — exact upstream parity, no silk formatting.
...
These `Commitlint` decisions are load-bearing for consumers:
...
- **A lint handler's byte-format step is a public static, not private handler-internal code, because two entry points share it.** Every `Lint` handler is reachable two ways — as a lint-staged handler (`<Handler>.create()`) and as a `savvy lint fmt <name>` CLI subcommand — and any formatting step that lives inside only one of them silently rewrites the file differently depending on which path ran. `Lint.PnpmWorkspace.formatContent` (`src/lint/handlers/PnpmWorkspace.ts`) is the worked example: it stringifies the sorted content through `@effected/yaml` under `DEFAULT_STRINGIFY_OPTIONS` (`indentSequences: true`, `quoteStyle: "double"`), and both `create()` and the CLI subcommand call it, so neither writes a differently-shaped file. Those options reproduce the repo's canonical byte format directly — the former Prettier post-process (which existed only to re-indent block sequences and re-quote single-quoted scalars) is gone as of `@effected/yaml 0.5.0`, which can now express both, so there is no second printer to drift from; `formatContent` dropped its now-unused `filepath` parameter but stays `async` for source compat. `Lint.PackageJson.sortContent` (`src/lint/handlers/PackageJson.ts`) is the same invariant over `@effected/package-json`'s `PackageJsonFormat.formatToString` (the tolerant text path — it sorts a private or version-less root rather than rejecting it, and returns unparseable content unchanged). When adding a handler that formats rather than merely checks, put the whole stringify/sort step behind one exported static and route both entry points through it. A regression test in `__test__/lint/index.test.ts` pins the resulting shape, and the operation must be idempotent — a second pass over an already-formatted file is a byte-for-byte no-op.
  - **A shared static is necessary but not sufficient once the step takes OPTIONS.** The `fmt` subcommand runs in its own process, so per-call options set on `<Handler>.fmtCommand(options)` reach the formatter only if they are serialized onto the emitted command line. `Lint.Yaml` is the worked example: `fmtCommand` encodes `options.format` through `Yaml.encodeFormatOptions` into a `--format` flag, and `savvy lint fmt yaml` decodes it with `Yaml.parseFormatOptions` before calling `formatFile`. Skip that hop and the two entry points silently write different bytes for the same configuration — the same drift the shared static prevents, reintroduced one layer up. A regression test asserts both paths produce byte-identical output for identical options; add one for any handler whose format step grows an options parameter.
- **`Lint.Yaml` runs on `@effected/yaml`; Prettier and `yaml-lint` are gone from the package.** The generic YAML handler (`src/lint/handlers/Yaml.ts`, every `**/*.{yml,yaml}` except the two pnpm files) formats arbitrary user-authored YAML where comment fidelity matters. It waited on effected#127 because `YamlFormat.formatToString` used to drop trailing and own-line comments; #127 landed in `@effected/yaml 0.8.0`, which round-trips comments and blank lines and formats every document of a multi-document stream. Four properties of the migrated handler are load-bearing and should not be undone casually:
  - **The statics are synchronous.** The engine is a pure, IO-free tier, so the only IO is the handler's own read and write. `Yaml.create()`'s returned handler therefore THROWS on invalid YAML rather than returning a rejected promise — a test asserting on rejection must use `expect(() => …).toThrow`, not `rejects`.
  - **`validateFile` gates on `Yaml.parseAllResult`, not `parseResult`.** A single-document parse sees only the first document of a stream, so a file whose SECOND document is invalid would pass silently. The old `yaml-lint` path rejected multi-doc outright, so whole-stream validity is the behavior being preserved, not a new feature.
  - **There is no config-file tier and none is planned.** `@effected/yaml` loads nothing from disk by design. The removed `.yaml-lint.json` discovery (`findConfig`/`loadConfig`) never resolved — neither search path existed in this repo — so the schema was always `undefined`. Formatting is configured in code through `YamlOptions.format`, defaulting to `quoteStyle: "double"` + `indentSequences: true`.
  - **`indentSequences` is the option that matters; `quoteStyle` is inert on this path.** `quoteStyle` governs only scalars the stringifier creates and never re-quotes scalars already in the source — measured against this repo's YAML corpus, it moved zero files, while `indentSequences: true` cut rewrites from 11/18 files to 1/18. It is set for consistency with `PnpmWorkspace`, not for effect.
...
`src/schemas/SavvySections.ts` centralizes the shell content the Silk Suite husky hooks share, so consumer CLIs no longer hand-write package-manager detection. Two consumers drive it: `@savvy-web/commitlint` (`savvy-commit`) and `@savvy-web/lint-staged` (`savvy-lint`). It exports `SavvyBaseSection`/`savvyBasePreamble()` (the package-manager-detection preamble: `ROOT`, `in_ci`, `detect_pm`/`PM`, `pm_exec`), `SavvyHooksSection`/`savvyHooksHygiene()` (self-guarded `core.fileMode` + `chmod +x`), `SavvyToolchainSection`/`savvyToolchainCheck()` (self-guarded package-manager drift warning) and `savvyToolSection(toolName, command)`.

`savvyToolchainCheck()` is the one block that is deliberately SELF-CONTAINED rather than composed on `savvy-base`. Its homes are `.husky/post-checkout` and `.husky/post-merge` — drift becomes true exactly when you pull a pin bump or switch branches — and those two hooks carry `SAVVY-HOOKS` but no `SAVVY-BASE`, so `ROOT`, `in_ci` and `PM` do not exist there. Adding the preamble to them to satisfy one block would put package-manager detection into hooks that otherwise need none; defining its own root/CI/pin lookups is the cheaper half of that trade. `.husky/post-commit` deliberately does NOT get it: it fires on every commit, which is noisier than the drift warrants.

It warns and never blocks — a hard failure would strand anyone mid-bisect or mid-rebase on an older pin — and it installs nothing, which is what separates it from `devEngines.packageManager.onFail: "warn"`. That flag is not a warning flag: pnpm responds by managing the package manager as a lockfile dependency, adding a `packageManagerDependencies` importer plus every platform binary. Every input is optional (no git root, no `jq`, no `devEngines`, a manager not on `PATH` all mean "say nothing"), only an exact pin is comparable so ranges and wildcards are skipped, the `+sha512…` integrity tail is stripped before comparison, and the `name` recorded in the pin decides which manager is asked rather than an assumption of pnpm.
...
- **`ReposConfigStore`** (`src/repos/services/config-store.ts`) reads and writes the validated manifest, failing with `ReposConfigError` (`kind: "missing" | "invalid"` — "missing" is the friendly, exit-0 case for a repo that vendors nothing). Beyond `exists`/`read`/`write` it owns **`update(root, fn)`**, a serialized read-modify-write: it acquires an exclusive-create lock file beside the manifest (`wx` open, retried on `AlreadyExists` with exponential backoff capped at 2s total — long enough to ride out a contending writer, short enough that a crashed holder's lock fails fast rather than hanging), reads the manifest (an absent one is handed to `fn` as `{ repos: {} }`, so `update` can initialize), applies `fn` (plain value or `Effect`), writes, and always releases the lock via `Effect.ensuring`. Every `ReposManager` manifest mutation goes through `update` rather than a bare read-then-`write`, so two concurrent callers queue instead of racing a lost update.
- **`ReposManager`** (`src/repos/services/manager.ts`) is the submodule plumbing, and now covers the whole vendored-repo lifecycle: `status`, `sync`, `add`, `pin`, `note`, `remove`, `rename`, `restore` and `deregister` — detailed in [Lifecycle operations](#lifecycle-operations-reposmanager) below. All git plumbing goes through `@effected/git`'s `Git`, whose typed failures are mapped onto this module's `GitSubmoduleError`. `ReposManager.layer` requires `ReposConfigStore | Git | FileSystem | Path | ReposLockdown`.
- **`ReposDrift`** (`src/repos/services/drift.ts`) is the read-only reconciler: `check(root)` compares the five authorities a vendored repo's state is spread across (and looks one level down into a vendored repo's own submodules) and reports every disagreement — see [Drift reconciliation](#drift-reconciliation-reposdrift). `ReposDrift.layer` requires `ReposConfigStore | Git | FileSystem | Path` — deliberately NOT `ReposLockdown`, because it never writes and runs unmodified against a locked tree.
- **`ReposLockdown`** (`src/repos/services/lockdown.ts`) is the boundary itself: `lock` chmods a vendored tree to files `0444` / directories `0555` (preserving each file's executable bit: `0555` instead when any of `0111` was already set), `unlock` restores `0644`/`0755` (`0755` for an executable), and the walk skips symlinks entirely (no chmod, no descent — there is no `lchmod` on Linux, and `fs.stat` following a symlink would otherwise touch whatever it points at). The two directions walk different roots on purpose: `lock` covers the worktree alone, `unlock` covers the worktree AND the module gitdir (see the scope decision below). Failures surface as `ReposLockdownError { path, reason }`. `ReposLockdown.layer` needs only `FileSystem | Path`.

Eight decisions in this area are load-bearing:
...
- **The lock covers the WORKTREE only; the submodule's git metadata directory is deliberately left writable.** Locking the metadata dir made the boundary enforce itself solely through an `EACCES` naming neither `.repos/` nor a reason, and it broke ordinary tooling: a plain `git pull` that moves a gitlink recurses by default (`fetch.recurseSubmodules=on-demand`) and dies writing `FETCH_HEAD`, and any client keeping per-gitdir state — GitKraken writes a `gk/` directory into every gitdir it manages, which no git setting governs — is structurally incompatible with a read-only gitdir. Verified against git 2.54, the worktree lock still blocks editing a vendored file and blocks `git reset --hard`, but does NOT block `git checkout <other>`, which moves `HEAD` while leaving the worktree stale. So the invariant is **"a drifted pin is always detected and one command from repaired"** (git reports `+<oid>`, drift reports `checkoutDiverged`, `restore` repairs), not "the pin cannot drift". `lock` and `unlock` are ASYMMETRIC on purpose and that asymmetry is the migration: `unlock` still walks the metadata dir so a tree locked by the previous implementation is freed, while `lock` never re-locks it — one `savvy repos sync` migrates an existing checkout.
- **The other half of the boundary is DECLARATIVE, written by both `sync` and `add`.** Both write `submodule.<path>.update = none` and `fetch.recurseSubmodules = false` into the superproject's LOCAL config — never `.gitmodules`, because this is a property of THIS checkout's workflow and must not be published to anyone cloning the repo. `update = none` makes `git submodule update`, `git pull --recurse-submodules` and every GUI client driving them skip a vendored tree deliberately, instead of discovering the boundary by failing against a permission error. `add` asserts it as well as `sync` because `add` is a creation point: leaving the marker to the next `sync` leaves a freshly vendored tree undeclared, and every client free to manage it, in exactly the window the marker exists to close. The two assertions differ in one way: `update = none` also makes `git submodule update --init` SKIP, so `sync`'s initialize call passes `checkout: true` — `--checkout`, git's documented command-line override of the marker, exposed by `@effected/git` since 0.8.0 (effected#333) — and the key itself is never rewritten mid-sync (the earlier flip-to-`checkout` bracket, its two extra config writes per repo, and the crash window that could leave the marker at `checkout` are all gone); `add` needs no override at all, since nothing in it runs `git submodule update`. `submodule.<path>.active` is deliberately left `true`, since an inactive submodule reads as *uninitialized* in `git submodule status` even when fully checked out — which would make every drift report claim `missingWorktree` — and `git submodule init` flips it back regardless.
- **The metadata dir is still derived from the checkout rather than assumed**, because `unlock` and `sync`'s stale-lock pass both need to land on the real one. `resolveModuleDir` reads `<root>/.repos/<name>/.git` — normally a FILE holding a `gitdir:` pointer — and resolves it (relative pointers against the `.git` file's directory), because git names `.git/modules/<path>` after the path the submodule was *registered* under, which need not be the manifest key: git never renames that directory, so an entry re-slugged after vendoring keeps its original gitdir name indefinitely. It degrades to the name-based path when the pointer is unreadable and never fails; it is also held to `path.relative(root, resolved)` staying inside `root` — a hand-edited pointer that resolves outside the repo falls back to the name-based path rather than letting the walk chmod something it shouldn't. That same derivation is what lets `ReposDrift` report `localRegistrationDivergence`: the gitdir's own path IS the registration name, so comparing it to the canonical section name detects the divergence precisely.
...
- **`withUnlocked(root, name, effect)` owns the whole finalizer contract itself; callers no longer re-lock explicitly.** It runs `unlock`, captures its `Exit`, runs `effect` only if unlock succeeded (restoring interruptibility around it), then ALWAYS runs `lock` afterward and captures its `Exit` too — including when unlock failed part-way or `effect` was interrupted, so the tree never sits open on an aborted path. A relock failure surfaces as `ReposLockdownError` only when it is the SOLE failure; an unlock or inner-effect failure always wins and the relock failure is swallowed behind it. `sync`/`add`/`pin`/`remove`/`restore` no longer issue a trailing `lockdown.lock(root, name)` — the write/`git add` work that happens after the unlocked window is covered by this same finalizer surfacing its own failure.
- **`rename` hand-rolls the same bracket contract rather than reusing `withUnlocked`, because the tree it unlocks and the tree it must relock are different paths.** It unlocks `oldName` (worktree present, `.git` pointer readable) and, once `git mv` succeeds, must relock `newName` — `resolveModuleDir` keyed on `oldName` post-move finds no worktree and degrades to a name-based fallback, so the worktree half of the lock walk would find nothing to lock. The implementation reproduces `withUnlocked`'s `Effect.uninterruptibleMask` contract (unlock, run, relock always attempted, an unlock or inner failure always winning over a relock failure) while tracking a mutable `lockName` that flips from `oldName` to `newName` the instant `git mv` returns: a failure before the move relocks the unmoved old tree, a failure after it relocks the moved new tree, and the happy path always ends locked under `newName`. The one deliberate divergence is that it does not thread the inner effect's success value out — the caller builds `ReposRenameResult` from the names itself, so the inner generator is typed `void`.
- **Every `manifest.repos[name]` membership read goes through the `Object.hasOwn`-guarded `getRepoEntry` helper, never a bare bracket read.** A manifest key literally named `"constructor"` or `"toString"` must fail typed as `RepoNotFoundError`, not resolve an inherited `Object.prototype` member — the same prototype-pollution-read hazard `ReleasePlanner` guards against for changelog module ids.

Consequence for consumers: `sync`, `add`, `pin`, `remove`, `rename` and `restore` all widen their error channel with `ReposLockdownError`; the corresponding CLI handlers `catchTag` the full union their `ReposManager` method can produce (log the message, `exitCode = 1`), so `reposCommand`'s own annotated error channel narrows to `GitSubmoduleError` — the one channel `status`/`status --drift` leave uncaught. In the MCP server `ReposLockdownError` stays in `repos_manage`'s error union. Reads (`status`, `ReposDrift.check`, `repos_inspect`) work unchanged against a locked tree — reading never needs write permission — and `.repos/config.json` is host-repo content that is never locked.

#### Lifecycle operations (`ReposManager`)

- **`status`** reports presence, working-tree dirtiness and stale note ids, plus an **index-aware commit triple** rather than a single commit: `stagedCommit` (the gitlink oid in the index via `git ls-files --stage`, filtered to mode `160000` — the only place a pin that is staged but not yet committed is visible), `committedCommit` (the oid at `HEAD` via `git ls-tree`), and `checkedOutCommit` (`git rev-parse HEAD` inside the submodule worktree). The deprecated `commit` alias of `stagedCommit` bridged one release and is now **gone** from `RepoStatusEntry`; consumers read `stagedCommit`. Failure discrimination is deliberate: an empty `ls-tree` listing is a legitimate non-error (a path not tracked at `HEAD` yet), but a failing invocation propagates as `GitSubmoduleError`, and only `NotARepositoryError` from `revParse` folds to "no checkout" — any other `revParse` failure is a present-but-corrupt repository, exactly what `status` exists to surface.
- **`sync`** clears stale locks (`index.lock`/`shallow.lock` older than `STALE_LOCK_MAX_AGE_MS`, ten minutes — a younger lock may still be held by a live git process), initializes absent checkouts `--init --depth 1`, and re-applies sparse patterns on every run. It now also **reconciles a `.gitmodules` submodule URL that has drifted from the manifest** (reported as `urlSynced`) and **registers an orphan manifest entry** that has neither a worktree nor a `.gitmodules` section (reported as `registered`). A section existing without a worktree is NOT an orphan — a prior `add` registers the section before materializing the checkout — so only the both-absent case registers from scratch. Two things join it here. It **asserts the declarative boundary on every entry on every run** (`boundaryMarked`), with the initialize call overriding the marker via `--checkout` on the command line rather than rewriting the key; the CLI deliberately neither logs `boundaryMarked` per entry nor folds it into the "up to date" idle check, because a field that is never empty is never news — it stays in the structured report for callers that want to verify the assertion. And it **deinitializes a vendored repo's own submodules before applying sparse patterns**, because sparse-checkout governs the parent's TRACKED files only: a materialized nested submodule survives `sparse-checkout set` untouched no matter what the manifest lists, so reporting `sparseApplied` without the deinit is a false success.
- **`add` is atomic, with remote ref validation up front.** The order is load-bearing: validate the effective name against `RepoName` (path traversal, separators, `.`/`..`) before any side effect; read the manifest, treating only `kind: "missing"` as absence so a transient I/O error can never reinitialize over a real manifest; reject a duplicate name; then `git ls-remote` the requested ref and fail with a **near-miss suggestion list** when it is unknown, so an unresolvable ref leaves no gitlink, no `.gitmodules` change and no worktree. Inside the unlocked bracket it detects **resumable partial state** (a prior `add` that staged the gitlink and section then died before fetch/checkout: same path + same url resumes by skipping `submodule add`; same path + a different url fails typed rather than overwriting someone else's partial work), and everything after that point is **rollback-guarded** — each rollback step's failure is logged and swallowed independently so one failure never skips the rest, and the ORIGINAL failure always propagates. The rollback resolves the module gitdir FIRST, while the worktree's `.git` pointer still exists: `submodule deinit --force` clears that pointer, so reading it afterwards silently degrades to the name-based fallback and would orphan a divergently-named prior registration's real gitdir while reporting a clean rollback. It also **declares the boundary itself** rather than deferring to the next `sync` (see the declarative-boundary decision above), and takes an optional **`orientation`** — the one parameter that makes remove-then-re-add, the standing remedy for several vendored-tree problems, lossless in one call. `add` resurrects nothing on its own, so without it that remedy silently destroys the entry's hand-curated orientation block, and nothing downstream notices: `status` and `drift` both report clean afterwards, because neither authority knows an orientation was ever there. Notes are deliberately NOT carried across — they are ephemeral by policy; orientation is meant to survive.
- **`pin`** re-pins an entry to a new ref; **`note`** adds, removes or promotes an agent note — `promote` now **appends** to the target document (`layout` or `startHere`) instead of overwriting it.
- **`remove`** unvendors an entry: `submodule deinit --force`, `git rm --cached`, then the worktree and module gitdir are deleted, the `.gitmodules` section dropped and the manifest entry removed. It resolves the module gitdir before deinit for the same reason `add`'s rollback does. The worktree/gitdir deletions run *inside* the unlock bracket deliberately — the relock walk over a now-missing directory is a silent success. The result carries `removedNotes` so any durable note can be promoted elsewhere before the removal is committed, plus a ready-made `commitMessage` — and **`removedEntry`, the whole manifest entry as it stood**, which exists for the re-vendor path: `removedEntry.orientation` hands straight back to `add`'s `orientation` parameter. `removedNotes` is exactly `removedEntry.notes ?? []` and is retained alongside it deliberately, because the two have opposite lifetimes (a last look at something ephemeral versus the durable block to carry forward).
- **`rename`** validates the new name and its non-collision before touching anything, then: `git mv` moves the worktree, stages the rename, rewrites and stages `.gitmodules`' `path` field and recomputes `core.worktree` (verified against real git 2.54 — none of those three are redone by hand); `core.worktree` is nevertheless **idempotently re-asserted** in the module's `config` and — when it exists — its `config.worktree`, because that value going stale was the original regression, and a `config.worktree` (never touched by `mv`) is ALWAYS broken immediately after the move rather than coincidentally stale. The VALUE matters as much as the invocation: git resolves a relative `core.worktree` against the GITDIR, so it is written as `path.relative(moduleDir, path.resolve(root, newRepoPath))` — `resolve` first, because `path.join` never normalizes a relative first segment and the CLI's `--cwd` defaults to literal `"."`, which is exactly the bare repo-relative value that broke a live repo. Both writes route `cwd` at `root` with an ABSOLUTE `-f` path into the module dir and never `cwd: moduleDir`: with `extensions.worktreeConfig` set, git's repository discovery reads the (untouched, now stale) `config.worktree` before running any subcommand and dies with "cannot chdir". The `.gitmodules` section NAME — untouched by `mv`, and, in a repo re-slugged after vendoring, never matching the manifest key to begin with — is canonicalized to the new repo path by locating the section via its already-updated `path` field. **`submoduleInit` then re-registers the superproject's own `.git/config` `submodule.<name>.*` keys under the new name AFTER that canonicalization** (git writes them only on `submodule add`/`init`, keyed by section name, so a renamed entry otherwise reads as uninitialized in `git submodule status` forever), and the stale old keys are unset tolerantly (`configUnset` fails loudly on a never-set key, and a never-initialized old registration is legitimate). Finally the manifest key is renamed through `configStore.update` and a `git submodule status` read verifies the whole move. **Known limitation, deliberate:** `rename` does NOT relocate the module gitdir on disk (`.git/modules/<old path>` stays where it is; only the worktree moves, which is why the single resolved `moduleDir` stays valid across the operation). The consequence is that vendoring a NEW repo under the freed old name hits git's refusal to reuse an existing local git directory — a typed failure, not silent corruption.
- **`restore`** hard-resets checkouts back to their gitlink commit and re-applies sparse paths. The **staged gitlink wins over the committed one** (a pin staged but never committed must not be discarded back to the last commit); with neither, it fails typed with "nothing to restore to". Called with explicit names it validates every name exists BEFORE resetting any of them (a typo in a later name must not leave earlier ones already reset) and restores each regardless of cleanliness — an explicit ask is always honored, so `skippedClean` is always empty on that path. Called with no names it restores every dirty repo from `status` and reports the clean ones in `skippedClean`. Two additions make it honest about what it achieved rather than what it attempted. It **deinitializes the vendored repo's own submodules before the reset**, because `git reset --hard` does not recurse and sparse cannot evict a nested checkout — a diverged nested tree would otherwise survive the reset, leaving the parent permanently dirty (`M <nested>`) while presenting source from an unpinned version; deinit rather than a recursive reset is the right repair, since the target state for a reference source is *no* nested checkout at all. And it **re-reads `git status` inside the unlock bracket afterwards** and reports any repo that is still dirty in **`stillDirty`** (the CLI logs a warning and exits 1; the MCP renders a "RESTORE DID NOT FULLY SUCCEED" section only when non-empty, so the heading never becomes background noise). Membership in both `restored` and `stillDirty` is the normal shape for such a repo — reporting a reset that ran over a tree that never came clean as plain success is what let a nested divergence look repaired.
- **`deregister`** (systems#463, effected#333) clears a STALE `submodule.<section>.*` registration from the superproject's LOCAL git config — the orphan `localRegistrationDivergence` a rename or an unvendoring leaves behind, which `git submodule status` lists as a phantom entry and which no other tool could fix (the drift report used to name a raw `git config --remove-section` its own guard had to be widened to permit). The `section` argument is the registration name exactly as the drift report states it (`submodule.` prefix implied). Four deliberate shapes: it **refuses** (typed `ReposConfigError`, before any mutation) a section outside `.repos/` entirely — a host repo's own submodule registration is not this machinery's to clear — as well as a live registration under EITHER name: the canonical `.repos/<key>` of a manifest entry, or a diverged name identified by the same module-gitdir attribution `ReposDrift` uses (`resolveModuleDir`'s path relative to `.git/modules` IS the real registration name; that state's remedy is a re-vendor, so clearing it would deregister a healthy checkout) — with one carve-out: a gitdir match whose entry's CANONICAL section is ALSO registered is a genuinely orphaned twin (gitdir location and registration identity are independent, and git never renames the gitdir), and removing it is a crashed-rename recovery's own final step, so it is allowed — reconciling a live registration is `sync`'s job and unvendoring is `remove`'s; it treats a MISSING manifest as empty, since a stale registration can outlive the manifest itself; and it **probes via `configList` before removing** — pinned to the repository-local config file (resolved through a linked worktree's `gitdir:`/`commondir` indirection), since a bare `configList` reads the merged system/global/local view and a global-only section would pass the probe then die loudly on the local-scoped removal; the file override is the interim for the kit's missing scope option (effected#372) — both so a typo surfaces as a typed nothing-to-deregister failure instead of git's loud exit-128 and so the result's `removedKeys` reports the achieved state (the keys the section actually carried, read before removal). Key→name attribution reuses drift's `submoduleNameFromKey` (exported `@internal`) so the report and the remedy can never disagree about a section's name. The removal itself is one `Git.configRemoveSection` call (`@effected/git` 0.8.0). It touches no vendored worktree, so it is the one mutating op with NO lockdown bracket (`ReposLockdownError` absent from its union) and NO `commitMessage` in its result — local config is unversioned, nothing is staged.

#### Drift reconciliation (`ReposDrift`)

`check(root)` reconciles five authorities — the manifest, `.gitmodules`, the worktree, `git submodule status`, and the superproject's LOCAL git config — and additionally looks one level down into each vendored repo's own submodules, returning a `ReposDriftReport { drifts, clean }`. Each `RepoDrift` carries `name`, a `kind`, a human-readable `detail`, and for value mismatches the disagreeing `manifestValue`/`observedValue` pair. The ten `DriftKind`s: `urlMismatch`, `pathMismatch`, `unregisteredManifestEntry`, `orphanGitmodulesEntry`, `missingWorktree`, `checkoutDiverged`, `missingShallow`, `gitmodulesUnparsable`, `localRegistrationDivergence`, `nestedSubmoduleDivergence`.

Five reconciliation decisions are load-bearing:

- **Sections are matched by their canonical NAME first, never by their `path` field.** `git submodule add` derives the section name from the path at creation time, so `section.name` normally equals `.repos/<manifest key>` — and matching on `path` would hide the `pathMismatch` drift this loop exists to report.
- **Name-pairing failure falls back to pairing by `path`, claiming each section at most once.** A section whose name diverged (a manifest entry re-slugged after the section was created) would otherwise degrade straight into an `unregisteredManifestEntry` + `orphanGitmodulesEntry` pair and mask the url/shallow comparisons for a repo that is really just misnamed. The fallback reports the divergence as `pathMismatch` and still runs the remaining checks. Any section left unclaimed at the end is an `orphanGitmodulesEntry`.
- **A `.gitmodules` parse failure is reported as the SOLE drift, not as an error.** Reconciliation is impossible against an undecodable document, and guessing per-entry state from one is worse than reporting `gitmodulesUnparsable` (whose `name` is `.gitmodules`, not a repo). An absent `.gitmodules` is different: every manifest entry is then reported `unregisteredManifestEntry`. `git submodule status` states map on: `uninitialized` (or an empty checkout) → `missingWorktree`, `outOfSync` → `checkoutDiverged`, and `conflict` → `checkoutDiverged` too, with detail text naming the unresolved merge conflict since `DriftKind` has no dedicated literal for it.
- **The superproject's local git config is the FIFTH authority, and the module gitdir's own path is what makes reading it precise.** `.git/config`'s `submodule.<name>.*` registration is unversioned, written only by `submodule add`/`init`, and keyed by section NAME, so nothing among the other four reads it: after a section canonicalization the manifest, `.gitmodules`, the index and the worktree can all agree — a clean report — while the checkout is still registered under the OLD name and `git submodule status` reads a perfectly healthy entry as uninitialized. Rather than guess, the check compares each entry's resolved module gitdir path (relative to `.git/modules`) against its canonical section name: git names that directory after the registration name in force at creation and never renames it, so the gitdir path IS the real registration name. A mismatch is `localRegistrationDivergence` whose `detail` names the `git submodule sync` + `git submodule init` fixup; an entry whose gitdir is not under `.git/modules` at all is skipped, since there is no registration name to compare. The same kind also covers the ORPHAN variant — a registered `submodule..repos/*` section matching no manifest entry and explaining no divergence above — but with a different stated remedy (`savvy repos deregister <name>`, the sanctioned wrapper over `git config --remove-section`), because `sync`+`init` re-registers something that no longer exists. Config keys are parsed by stripping the fixed `submodule.` prefix and the final `.<property>` segment, never by splitting on `.`: a submodule name is routinely a PATH and contains dots itself.
- **A vendored repo's own submodules are checked one level down, and a failure there is swallowed rather than propagated.** `sparse` governs only the parent's tracked files, so a manifest `sparse` list never covers gitlink entries and an initialized nested submodule sits outside every authority above — which is how a repo pinned at one version presents source from another, the exact failure vendoring exists to prevent. Any nested entry whose state is neither `current` nor `uninitialized` is reported as `nestedSubmoduleDivergence` pointing at `restore`. The probe uses `Effect.orElseSucceed(() => [])`: this is extra depth on a report whose other kinds are already computed, and a nested-status hiccup must not turn the whole drift report into an error (a vendored tree with no `.gitmodules` of its own already yields an empty listing rather than a failure).

Consumers: `savvy repos status --drift` and the MCP `repos_inspect` `drift` mode (see `../cli/architecture.md` and `../mcp/architecture.md`), plus the plugin's `gitmodules-drift` monitor (`../silk/plugin.md`).
...
Value objects in this package implement `Equal.Equal` and `Hash.Hash` for structural comparison. Two patterns are used:

**Schema-based** (preferred for serialisable types): Extend `Schema.TaggedClass`. Override `[Equal.symbol]` and `[Hash.symbol]` to control comparison semantics — `AnalyzedWorkspace` is the surviving worked example, comparing on identity fields while its `linked`/`fixed` cross-references stay out of the hash to avoid the cycle.

**Plain class** (for non-serialisable types with complex construction): Implement `Equal.Equal` directly with a private constructor and a static `make()` factory. Reach for this when a field is function-valued (a tagged enum with behavior, a comparator) and so cannot round-trip through Schema.

Both patterns are now mostly *consumed* rather than defined here: the section, tool and versioning value objects moved to the kit (see [What the kit owns now](#what-the-kit-owns-now)), and the kit's own value classes follow the same Equal/Hash discipline.
...
Consumers match on these with `$is`. The section outcomes silk-effects used to define this way now come from `@effected/templates` in a flattened form (`CheckOutcome` is `UpToDate`/`Drifted`/`Absent`, not a nested `Found` variant carrying a boolean), so a branch written against the old three-state shape does not typecheck — it silently changes meaning only if the compiler is bypassed.
...
- `SilkPublishability.layerAdaptive` (also requires `ChangesetConfig`; its vanilla branch reads the `PublishabilityDetector.npm` **value** — the kit deleted the bare `.layer` static, because policies are values)
...
- `SilkWorkspaceAnalyzer` / `SilkWorkspaceAnalyzer.layer` Depends on: `FileSystem`, `WorkspaceDiscovery`, `PackageManagerDetector`, `ChangesetConfigReader`
- `Changesets.DepsRegen` / `Changesets.DepsRegen.layer` Depends on: `WorkspaceSnapshots`, `ConfigInspector`, `WorkspaceDiscovery`, `PublishabilityDetector`, `ChangesetConfig`, `Git`, `FileSystem` (`WorkspaceSnapshots` + `PublishabilityDetector` + `Git` from `@effected/workspaces`; all changeset file I/O goes through the `FileSystem` Tag, resolved once at construction so `plan`/`execute` stay `R = never`)
- `Changesets.DepsRegenDefault` (batteries-included) Requires only the platform services — `FileSystem | Path | ChildProcessSpawner`. Composes the kit graph via `Changesets.makeChangesetKitGraph` (config-dependency hook replay in catalog assembly, subprocess variant — see [Tool Namespaces](#tool-namespaces-changesets-commitlint-lint)), plus `ConfigInspector.layer` and `SilkPublishability.layerAdaptive` internally; `WorkspaceSnapshots` reads git history, so the platform layer must be spawn-capable (`NodeServices.layer`)
- `Changesets.ReleasePlanner` / `Changesets.ReleasePlanner.layer` Depends on: `ConfigInspector`, `FileSystem` (the `FileSystem` backs the preview path's `Scope`-managed temp dir)
- `Changesets.ChangelogService` — requires `GitHubService` only; the `MarkdownService` indirection that used to sit in this channel is deleted

**Pure modules (no platform requirements):**

- `SilkPublishability` (all-static class — `detect` is pure; `resolveTargets`/`listPublishable` are Effects requiring the `PublishabilityDetector` Tag; `readTargetsBinding` is an `Effect` requiring `FileSystem`)
- All value objects and tagged enums

Kit services silk-effects consumers commonly need alongside these — `ManagedSection` (`@effected/templates`), `ToolDiscovery` (`@effected/commands`), `WorkspaceDiscovery`/`PackageManagerDetector`/`WorkspaceRoot` (`@effected/workspaces`) — are provided from the kit packages directly, not from here.
...
**Service tests that need a `FileSystem` build an `@effected/memfs` volume, not a hand-rolled stub**, and inject faults with `MemoryFileSystem.layerFaulty` where a permission or IO failure is the subject. The rules — why the stubs this replaced asserted less than their names claimed, the per-*build* volume-sharing trap, and the fault-injection idiom — are suite-wide and live in [../testing/effect-vitest.md](../testing/effect-vitest.md#filesystem-doubles-effectedmemfs). What is package-specific here is the carve-out: **memfs records mode bits but never enforces them**, so the `ReposLockdown` blocks that assert the OS genuinely refusing an operation, and the `it.live` lock tests in `__test__/repos/services__config-store.test.ts` that turn on real filesystem atomicity (`open(flag:"wx")`) and real mtime staleness, stay on real temp directories on purpose. Do not "finish the migration" by moving them.
...
The library is consumed by GitHub Actions (Node.js), CLI tools (Node.js), and potentially Bun-based tools. Depending on effect's platform abstractions (`FileSystem`, `ChildProcessSpawner`, in-core since v4) rather than on Node APIs ensures compatibility across all runtimes without requiring separate implementations.
