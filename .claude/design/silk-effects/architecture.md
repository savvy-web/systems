...
- **Worktree callers get re-rooted paths, but names/versions stay layer-bound** (#487). A long-lived host builds the discovery layer ONCE for its startup root, yet a caller in a git worktree of the same repository inspects a DIFFERENT `projectDir` — and an agent worktree (`.claude/worktrees/*`) is NESTED inside the primary checkout, so mixing primary workspace dirs with worktree file paths made the shadowing check see every worktree path as "inside" the primary root. When a discovered package's path does not sit inside `projectDir`, `inspect` re-roots it onto the per-call project dir via the kit's own `WorkspacePackage.relativePath` (`"."` for the root package), so the whole inspection speaks `projectDir` paths consistently; identity when the roots agree, and a discovery double without `relativePath` keeps its literal paths. Documented limitation: the kit has no per-call-root discovery, so package names/versions still come from the layer-bound root's manifests — the sound approximation for a worktree of the same repository.
