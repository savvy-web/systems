...
- **`status`** reports presence, working-tree dirtiness and stale note ids, plus an **index-aware commit triple** rather than a single commit: `stagedCommit` (the gitlink oid in the index via `git ls-files --stage`, filtered to mode `160000` — the only place a pin that is staged but not yet committed is visible), `committedCommit` (the oid at `HEAD` via `git ls-tree`), and `checkedOutCommit` (`git rev-parse HEAD` inside the submodule worktree). The legacy `commit` field survives as a deprecated alias of `stagedCommit`, retained for one release so `mcp`'s `repos-inspect` renderer and the CLI's status renderer can migrate; `null` preserves its prior "not staged" contract. Failure discrimination is deliberate: an empty `ls-tree` listing is a legitimate non-error (a path not tracked at `HEAD` yet), but a failing invocation propagates as `GitSubmoduleError`, and only `NotARepositoryError` from `revParse` folds to "no checkout" — any other `revParse` failure is a present-but-corrupt repository, exactly what `status` exists to surface.
