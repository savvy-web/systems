...
`SilkRuntimeLive` exposes three services: `SilkWorkspaceAnalyzer`, `WorkspaceRoot` (for root walk-up — see [Root Resolution](#root-resolution)) and `Turbo.TurboInspector` (backing `turbo_inspect`). It is built by providing `SilkWorkspaceAnalyzerLive` + `WorkspaceRootLive` + `Turbo.TurboInspectorLive` (the inspector fed its own `ToolDiscoveryLive`) with a `DepsLive` merge of `WorkspacesLive` (the workspace trio plus `DependencyGraph` and `TopologicalSorter` the analyzer needs), `ChangesetConfigReaderLive`, `TagStrategyLive` and `VersioningStrategyLive` (itself fed `ChangesetConfigReaderLive`, since `Layer.mergeAll` does not cross-feed siblings). `ToolDiscoveryLive`'s `PackageManagerDetector` + `WorkspaceRoot` requirements are satisfied by `DepsLive`; the leftover `CommandExecutor` + `FileSystem` + `Path` flow up to the host platform layer, which `bin.ts` supplies at the edge via `NodeContext.layer`. See `src/runtime.ts` for the exact wiring.
