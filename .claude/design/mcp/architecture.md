...
`SilkRuntimeLive` exposes the services the tools need: `SilkWorkspaceAnalyzer` (for `workspace_info`), `WorkspaceRoot` (for root walk-up, see [Root resolution](#root-resolution)), `Turbo.TurboInspector` (for `turbo_inspect`) and the `Changesets.BranchAnalyzer` + `Changesets.ConfigInspector` pair (for `changeset_inspect`). A single shared `ConfigInspector` instance feeds the branch analyzer and is re-exposed, the same shared-instance pattern the CLI uses. The leftover platform requirements (`CommandExecutor`, `FileSystem`, `Path`) flow up to the host layer, which `bin.ts` supplies at the edge via `NodeContext.layer`. See `src/runtime.ts` for the exact wiring.
...
`changeset_inspect` (`src/tools/changeset-inspect.ts`) follows the exact same pattern, backing the silk plugin's `changeset-manager` workflow. Its result is a **discriminated union keyed by `mode`** (`branch` | `config`): `branch` wraps `Changesets.BranchAnalyzer.analyzeBranch` (diff-against-base file classification) and `config` wraps `Changesets.ConfigInspector.inspect` (the resolved `.changeset/config.json`). Both embedded schemas bridge safely. The structured MCP result exists because the CLI's `--json` output is prefixed with an `Effect.log` line that breaks naive `JSON.parse`; the structured result sidesteps that fragility. See `src/tools/changeset-inspect.ts` and `../silk/plugin.md`.
