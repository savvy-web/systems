...
The repo-wide testing contract for Effect code: which test entry point to reach for, how layers get provided, how a `FileSystem` is doubled, how failures are asserted and the `TestEnv` behaviors that silently change what a test observes.
...
- [Test doubles fail loudly](#test-doubles-fail-loudly)
...
Test files that run Effect programs use `@effect/vitest` (`catalog:effect`), not plain `vitest` plus a manual `Effect.runPromise`. The library supplies vitest-integrated test entry points (`it.effect`, `it.live`, `layer`) that build and tear down an Effect runtime per test, so a test body is an `Effect.gen` block rather than a promise-returning closure over a hand-rolled runner. The vendored source at `.repos/effect/packages/vitest` is the authority on what the library exports.

The whole suite runs under the root `vitest.config.ts`: `@vitest-agent/plugin` discovers every package and `e2e/*` harness as a vitest project, the pool is `forks`, `@e2e/*` projects run with `fileParallelism: false` and `vitest.setup.ts` runs `turbo run build:dev` as the global setup so built `dist/dev` artifacts exist before any test that consumes one. The `plugins/silk` hook shell suite (bats + shellcheck under `plugins/silk/tests`, run by `pnpm test:hooks`) sits outside vitest entirely and none of this doc applies to it.

## Where `@effect/vitest` applies

**The rule is per-file, not per-package: a test file that runs an Effect uses `@effect/vitest`; a test file with no Effect surface stays on plain `vitest`.** A large share of the suite is in each camp and that split is expected rather than unfinished work — pure-function tests (formatters, path math, schema shape assertions, config validators) gain nothing from an Effect runtime and should not pay for one.

Consequently `@effect/vitest` is a devDependency of exactly the packages that have at least one Effect-running test file. A package whose *source* is Effect-based can still carry no such dependency — the bundler is the case in point, because its Effect surface runs behind a promise boundary the tests call across — and the `e2e/*` harnesses stay on plain `vitest` because they exercise built artifacts through their published entry points (see [`e2e` architecture](../e2e/architecture.md)).
...
# Live Effect runners in test files
grep -rn "Effect\.runPromise\|Effect\.runSync\|runPromiseExit" --include="*.test.ts" packages e2e
```

The second command is the invariant worth keeping green: **no test body runs a live runner.** The hits it is allowed to return are comments and `beforeAll`/`afterAll` hooks — `@effect/vitest` wraps test bodies, not vitest's lifecycle hooks, so a hook that has to run an Effect (unlocking a real tmpdir before `rmSync`, say) legitimately calls `Effect.runPromise`. Use `runPromise` there, never `runSync`: the platform filesystem operations are async, and `runSync` throws `AsyncFiberError` out of the hook and fails the whole suite. `packages/silk-effects/__test__/repos/services__lockdown.test.ts` is the worked example.
...
Two shapes justify `layer(...)`, and the suite uses both. Re-derive the current sites with `grep -rn "^layer(" --include="*.test.ts" packages`; a test that needs a *different* layer from its group's is kept outside the block rather than re-provided inside it, and the files that do this say so in-file.

**The layer is stateless.** The common case: `layer(Logger.layer([]))` wrapping a group only to silence a command's INFO logging, or a pure service layer (a validator, a report pipeline) with no mutable backing. An empty logger set carries nothing from one test to the next, so memoization is unobservable. The second precondition still has to hold and is asserted in-file at each site — the group must not depend on ambient process state either. These suites never `chdir`; each test drives a freshly-created temp dir passed in as an argument.
...
**A test that needs a `FileSystem` provides a real one over an in-memory volume — `MemoryFileSystem.layerWith(seed)` from `@effected/memfs` — not a hand-rolled stub.** Neither an `as unknown as FileSystem.FileSystem` object cast nor `FileSystem.layerNoop` with a handful of methods filled in is acceptable for code that touches a tree. Re-derive the current footprint rather than trusting a list: `grep -rl "@effected/memfs" --include="*.test.ts" packages`.

A hand-rolled stub is not merely verbose, it is *dishonest*, and each way it lies has a matching failure the volume removes:
...
- **`layerNoop` is deny-by-default, so isolating one method costs a whole fake tree.** Filling in `readDirectory: () => Effect.succeed([])` to get a recursion test off the ground hands it an *empty* tree to recurse into, which leaves the recursion-order invariant the test is named for unreachable behind a green result.

`FileSystem.layerNoop` remains acceptable only where the code under test never touches a tree and the layer exists to satisfy a type requirement.
...
Do not assert a failure through `Effect.exit` plus `Exit.isFailure` when the failure is supposed to be typed: that shape also passes when the error escapes as a **defect** — exactly the regression a typed-error codebase most wants to catch — and the nested `is-a-Failure` / `is-a-Some` guards it needs have a failure mode of their own, because an assertion nested under a guard is *silently skipped* whenever the guard does not hold, so a test can report green having asserted nothing.

**Defects** — where the point of the test is that something escapes the typed channel — use `Effect.exit` and inspect the `Cause`, with an explicit `throw` on the non-failure branch so a passing effect cannot fall through as a pass. `Cause.pretty` gives a readable message. `packages/silk-effects/__test__/changesets/utils__version-files.test.ts` has a worked example.

## Test doubles fail loudly

**A new test double must fail loudly on an unstubbed member** — `Layer.mock`'s `UnimplementedError` (see `Layer.mock` in `.repos/effect/packages/effect/src/Layer.ts`) or an explicit `Effect.die`. A double whose unstubbed members quietly succeed lets a consumer test stay green while exercising a path the double never modeled, and that lie is invisible until the real service disagrees. Where a service ships its own `*Test` layer, that layer is the double to use; a per-file double is for the shape the service does not provide.
...
Every convention above is in force across the suite: Effect-running test files are on `@effect/vitest`, no test body runs a live runner, typed failures are asserted with `Effect.flip` and `vi.mock` files import `vi` from `"vitest"`. Suite-boundary `layer(...)` blocks exist in several packages, each falling into one of the two justified shapes.

`@effected/memfs` is a devDependency of the packages whose tests need a `FileSystem` (re-derive with `grep -l '"@effected/memfs"' packages/*/package.json`). A few `FileSystem.layerNoop` sites remain where the code under test never touches a tree; adopt a volume when a new or edited test needs one rather than treating the remainder as a scheduled sweep.

## Rationale

**Why `@effect/vitest` rather than hand-rolled runners.** Per-file `run`/`runExit`/`runWith` closures are near-identical, individually divergent and each one is a place for the runtime's error handling to differ from its neighbors. Pushing runtime construction into the test framework makes the test body the only thing a test file has to get right, and gives the whole suite one place where `TestEnv` semantics are defined.
