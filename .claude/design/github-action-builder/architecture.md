...
The design is Effect-first. Four services define interfaces with `Context.Tag`; live layers provide implementations; the CLI consumes services directly while the `GitHubAction` class wraps them with `ManagedRuntime.runPromise` for non-Effect consumers.
...
Layer composition lives in `src/layers/app.ts`: `ValidationLayer` and `BuildLayer` each provide `ConfigServiceLive`, `PersistLocalLayer` stands alone and `AppLayer` merges all four.
...
All errors use `Data.TaggedError` for type-safe pattern matching via `Effect.catchTags`. The tagged union groups are defined in `src/errors.ts`: `ConfigError`, `ValidationError` (which includes `WorkerEntryMissing`), `BuildError` and `PersistError` (aggregated as `AppError`). Each error carries contextual data (paths, causes, expected-vs-specified paths) — read the file for the exact field shapes.
