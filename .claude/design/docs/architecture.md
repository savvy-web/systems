...
- **The `tools:` allowlist is exhaustive.** Claude Code treats an explicit `tools:` list as the complete set — unlisted tools are unavailable. Because the agent *must* read `silk://catalog` and call the server tools, the MCP tool ids (`ListMcpResourcesTool`, `ReadMcpResourceTool`, `mcp__plugin_docs_savvy-mcp__silk_docs_search`, `mcp__plugin_docs_savvy-mcp__workspace_info`) are listed alongside the editing and `Bash(... *)` tools it needs. Bash permissions use the **space-glob** form `Bash(git *)`, the canonical form — not the colon form `Bash(git:*)`.
