#!/usr/bin/env bash
# run-hook-tests.sh — single entry point for the github-actions plugin's hook
# test suite.
#
...
#    hook-debug.sh) carry no shebang; they are validated IN CONTEXT via `-x`
#    (follow-source) from the scripts that source them. `-P SCRIPTDIR` resolves
#    each `# shellcheck source=` directive relative to the sourcing script's
#    own directory.
