...
3. **Read the artifact.** Read `<package-dir>/dist/prod/issues.json`. This file is written on every terminal path, including a crash, so check `buildOk` before anything else: `false` means the build did not complete (see `failure.message`) — fix the crash and rebuild before trusting the diagnostic buckets, which may be empty despite the failure; a missing `buildOk` field means the artifact predates the stamp — treat it as unknown, not as a pass, and rebuild. Only once `buildOk: true` is confirmed, filter `warnings`/`errors` to entries whose `code` starts with `ae-` or `tsdoc-`. If the file is absent, the build did not run — rebuild. If `buildOk: true` and those arrays are empty, the package is already clean; report and stop.
...
5. **Verify.** Rebuild (`build:prod --filter <package> --force`) and re-read `issues.json`. Confirm `buildOk: true` before trusting the re-read diagnostics — a crash on the fix pass can leave the filtered arrays empty without the package being clean. Repeat fix→rebuild until `buildOk: true` and the filtered `ae-*`/`tsdoc-*` arrays are empty, or until only a genuine `@beta`/`@alpha` maturity call remains — for that, ask the user (`AskUserQuestion`) rather than guessing.
