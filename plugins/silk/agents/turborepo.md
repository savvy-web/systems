...
- **You do not recommend building inside an install hook.** `prepare`/`postprepare` builds are forbidden in this monorepo — the `build:prod` chain resolves `catalog:silk:peers` from a state file pnpm writes only after install completes, so an install-time build fails. Build is always an explicit post-install `pnpm build` step.
