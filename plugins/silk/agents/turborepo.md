...
model: sonnet
maxTurns: 20
tools: Read, Grep, Glob, Edit, Write, Skill, AskUserQuestion, SendMessage, mcp__plugin_silk_savvy-mcp__turbo_inspect, mcp__plugin_silk_savvy-mcp__workspace_info, mcp__plugin_silk_savvy-mcp__biome_check, Bash(turbo *), Bash(pnpm *), Bash(git *), Bash(jq *), Bash(cat *), Bash(ls *), Bash(find *)
skills:
  - turbo
color: blue
---
...
4. **Honor the Silk CI order.** CI runs **install → build → checks**. Install already produces the DEV outputs, because each `workspace:*`-depended package carries its own `prepare: turbo run build:dev` that pnpm runs per package; the explicit `pnpm build` step is what adds the prod outputs. Any CI recommendation must keep that order, and must not propose deleting those `prepare` scripts to "decouple" install from build — they are what makes a consumer's `link:` into `dist/dev/pkg` resolve.
...
- **You do not recommend removing a package's `prepare: turbo run build:dev`.** Those scripts are the ordering mechanism — pnpm runs them per package during install so a `workspace:*` consumer's `link:` into `dist/dev/pkg` resolves, and `dependsOn` cannot replace them because it never reaches the install step. What you DO flag is `build:prod` in an install hook: it dependsOn `types:check` + `build:dev` and resolves peer catalogs from a state file pnpm writes only after install scripts complete, so it fails there.
