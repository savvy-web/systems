...
| `lastMail.in` / `lastMail.out` | Receiver-repo-relative path to the most recent inbound mail; counterpart-repo-relative (i.e. starting `../../`) path to the most recent outbound mail. The monitor's new-mail detection keys off `lastMail.in`. `journal-append.sh` enforces this at append time (savvy-web/systems#546): `--mail-in` must resolve to an existing file under the repo root, with a bare filename (no `/`) normalized to `.claude/dogfood/<counterpart-id>/<file>` when that path exists, and rejected otherwise; the resolved path must also stay physically under the repo root (`..` traversal and symlinked directories are rejected), and the mail file itself must be a regular file, not a symlink; `--mail-out` is rejected only when it is a bare filename with no `/` at all (existence is never checked, since the counterpart's own layout may differ). A dangling `lastMail.in` — a stat that fails to resolve even after this fix, e.g. on a journal written before it existed — makes the monitor's watermark fall back through a second retry against the journal's counterpart mailbox before giving up to the loop's `loop-started` time. |
