...
generated:
  ...
  body_sha256: 18112a5dc51a2ab3ca0e52f7980cf60c97214b1fb9ae34d6b50056a3bed82084
---
...
**Guards.** The layering policy moved to `packages/silk/layers.json` on the kit's `LayerPolicy` schema, checked by `WorkspaceLayering.checkWorkspace` in `packages/silk/__test__/package-layering.test.ts`; `@e2e/workspace` is deleted.[^layering-test] `SourceBoundary` pins the cli/silk/mcp non-import invariant at source in silk, and the "only `bin.ts`/`main.ts`/`version.ts` touch `process`" rule in mcp.
