...
Load when working on `@savvy-web/tsdown-plugins` or the bundler's build behaviors. Covers the interface-only (no tsdown runtime / no peer) boundary, the Effect service and helper map, entry detection, manifest emission + catalog delegation, the dts resolved-tsconfig port, the per-TargetGroup build loop, the output reporter, the `src/meta/` API Extractor meta-generation pipeline (`generateMeta`, multi-entry api-model merge, deterministic tsdoc.json, message suppressor, `MetaGenerationError`), and the published escape-hatch contract.
