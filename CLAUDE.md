...
- Verify any Effect API against the vendored `.repos/effect` source or the installed beta, never v3 memory. Vendored trees are OS-level read-only (`Repos.ReposLockdown`); reads need nothing, and an `EACCES` there means route the change through `savvy repos` / the `repos_manage` MCP tool — never `chmod` the tree back by hand.
