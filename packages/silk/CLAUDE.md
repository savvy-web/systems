...
- Export map covers `./changesets/markdownlint`, `./commitlint`, `./lint`, `./biome`, and the `./tsconfig/*` presets. The long-deprecated `./changesets` (+ `/changelog`, `/remark`) and `./commitlint/{static,prompt,formatter}` subpaths are REMOVED — the changelog generator lives in `@savvy-web/changelog`, and silk-effects owns the rest.
- Carrier bins: `bin.savvy` → `src/bin/savvy.ts` and `bin["savvy-mcp"]` → `src/bin/savvy-mcp.ts`, shims over `@savvy-web/cli/main` and `@savvy-web/mcp/main` (each imports `main` and calls it with `{ distribution: { name: "@savvy-web/silk", version: process.env.__PACKAGE_VERSION__ } }`, nothing else — so `savvy --version` and the MCP `serverInfo.version` end in `via @savvy-web/silk <version>`; guaranteed under pnpm only, since npm's flat `.bin` may link a front end's own bin instead). Installing silk alone creates `node_modules/.bin/savvy` and `.bin/savvy-mcp` off that single direct dependency, no hoisting. The built shims keep the literal import specifiers (cli/mcp are declared deps, so tsdown externalizes them); `__test__/externals.test.ts` pins the built `bin` map and each shim's single import, `__test__/e2e/bins.e2e.test.ts` runs the built bins.
- ESM-only, every entry, no overrides: the Changesets CLI (v3), markdownlint-cli2 (0.23+) and commitlint (v19+) all `import()` their config modules, so no entry ships a CJS twin and nothing force-bundles `@savvy-web/silk-effects`. `externals.test.ts` asserts no `.cjs` is emitted and that the entries import silk-effects externally.
- Every entry externalizes `@savvy-web/silk-effects` (a declared runtime dependency), `semver`, and `source-map-support`; `effect`/`@effect/platform` are externalized in the dts only.
...
- `@effected/templates` is a direct runtime dependency: the `./lint` entry's declarations name kit `Section`/`SectionId` types, so it must stay a declared `dependencies` entry (the default manifest transform publishes `dependencies` as-is; there is no longer a custom keep-list in `savvy.build.ts`). Any package whose emitted `.d.ts` references a type must ship that package as a real dependency — dropping it breaks consumer typecheck under pnpm's strict layout.
...
- Load-bearing dependencies (systems#631): `@savvy-web/silk-effects` (nine `src/` importers), `@savvy-web/cli` and `@savvy-web/mcp` (the shim import targets), `@savvy-web/changelog` (a pure carrier dep — nothing imports it; it rides silk so the changesets engine can resolve it by id from the consumer root), `semver`, `@effected/templates`, AND `@effected/commands`, `@effected/git`, `@effected/workspaces`, `effect` — no file under `src/` imports those four directly; they satisfy `@savvy-web/silk-effects`' `peerDependencies`, and removing one breaks installs at install time, not at any lint pass. The non-import invariant still holds for library code: `src/bin/savvy.ts` and `src/bin/savvy-mcp.ts` are the ONE sanctioned place silk imports `@savvy-web/cli` / `@savvy-web/mcp`; nothing else under `src/` may.
- The Biome asset lives under top-level `public/`.
- `layers.json` (package root) is the WHOLE repo's layering policy on `@effected/workspaces`' `LayerPolicy` schema (`layers`, `tooling`, `unconstrained`, `requiredEdges`); it lives here because silk is the top of the graph. `__test__/package-layering.test.ts` holds the live workspace to it with `WorkspaceLayering.checkWorkspace` (an edge is any dependency NAME that is a workspace package, in all four dependency fields), and `__test__/boundaries.test.ts` pins the non-import invariant at source with `SourceBoundary`, waiving only `bin/savvy.ts` and `bin/savvy-mcp.ts`.
→ `@../../okf/interfaces/layers-json.md`
Load when editing `layers.json`, adding a package, or adding a dependency edge between workspace packages.
