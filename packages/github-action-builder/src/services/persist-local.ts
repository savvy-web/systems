...
function validateActionYmlPaths(actionYmlPath: string, destDir: string): Effect.Effect<void, ActionYmlPathError> {
	return Effect.gen(function* () {
		...
		const parsed = (yield* Yaml.parse(content).pipe(Effect.catch(() => Effect.succeed(null)))) as {
...
export class PersistLocalService extends Context.Service<PersistLocalService, PersistLocalServiceShape>()(
	 ...
	 * @public
	 */
	static readonly layer: Layer.Layer<PersistLocalService> = Layer.succeed(this, {
