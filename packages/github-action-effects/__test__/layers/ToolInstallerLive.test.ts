...
const run = <A, E>(effect: Effect.Effect<A, E, ToolInstaller>) => Effect.provide(effect, ToolInstallerLive);

const runFail = <A>(effect: Effect.Effect<A, ToolInstallerError, ToolInstaller>) =>
	Effect.flip(Effect.provide(effect, ToolInstallerLive));
...
describe("ToolInstallerLive", () => {
	describe("find", () => {
		it.effect("returns Option.none() for non-existent tool", () =>
			Effect.gen(function* () {
				const result = yield* run(Effect.flatMap(ToolInstaller, (svc) => svc.find("nonexistent-tool", "99.99.99")));

				expect(Option.isNone(result)).toBe(true);
			}),
		);

		it.effect("returns Option.some(path) for existing tool directory", () =>
			Effect.gen(function* () {
				// Create a fake tool cache structure
				const toolCacheDir = join(tempDir, "fake-tool", "1.0.0", process.arch);
				yield* Effect.promise(() => mkdir(toolCacheDir, { recursive: true }));
				...
				try {
					...
					const result = yield* run(Effect.flatMap(ToolInstaller, (svc) => svc.find("nonexistent", "0.0.0")));
					expect(Option.isNone(result)).toBe(true);
				} finally {
		...
		it.effect("returns Option.none() when path exists but is not a directory", () =>
			Effect.gen(function* () {
				...
				yield* Effect.promise(() => mkdir(join(defaultCacheDir, "file-tool", "1.0.0"), { recursive: true }));
				yield* Effect.promise(() => writeFile(toolPath, "i am a file not a dir"));

				try {
					const result = yield* run(Effect.flatMap(ToolInstaller, (svc) => svc.find("file-tool", "1.0.0")));
					expect(Option.isNone(result)).toBe(true);
				} finally {
					yield* Effect.promise(() => rm(join(defaultCacheDir, "file-tool"), { recursive: true, force: true }));
				}
			}),
		);

		it.effect("returns Option.some(path) when the tool directory exists in the default cache", () =>
			Effect.gen(function* () {
				...
				yield* Effect.promise(() => mkdir(toolPath, { recursive: true }));

				try {
					const result = yield* run(Effect.flatMap(ToolInstaller, (svc) => svc.find("cached-tool", "3.0.0")));
					expect(Option.isSome(result)).toBe(true);
					if (Option.isSome(result)) {
						expect(result.value).toContain("cached-tool");
					}
				} finally {
					yield* Effect.promise(() => rm(join(defaultCacheDir, "cached-tool"), { recursive: true, force: true }));
				}
			}),
		);
	});
	...
	describe("download", () => {
		it.effect("fails with ToolInstallerError for invalid URL", () =>
			Effect.gen(function* () {
		...
		it.effect("fails with ToolInstallerError when HTTP response is not ok", () =>
			Effect.gen(function* () {
				const { PassThrough } = yield* Effect.promise(() => import("node:stream"));
		...
		it.effect("follows HTTP redirects", () =>
			Effect.gen(function* () {
				const { PassThrough } = yield* Effect.promise(() => import("node:stream"));
		...
		// `it.live`, not `it.effect`: "Socket timeout" satisfies `isRetryableDownloadError`
		// (src/layers/ToolInstallerLive.ts:205), so `download` walks its
		// `Schedule.exponential("1 second")` retry (times: 2). Nothing here advances a
		// clock, so under a frozen TestClock the first backoff never elapses — verified,
		// it failed with "Test timed out in 10000ms". The real-clock cost is ~3s, which is
		// why the test already carried its own 10s budget.
		it.live(
			"fails with ToolInstallerError on socket timeout",
			() =>
				Effect.gen(function* () {
					const { PassThrough } = yield* Effect.promise(() => import("node:stream"));
		...
		it.effect("downloads a file successfully to a temp path", () =>
			Effect.gen(function* () {
				const { PassThrough } = yield* Effect.promise(() => import("node:stream"));
	...
	describe("extractTar", () => {
		it.effect("extracts a tar.gz archive", () =>
			Effect.gen(function* () {
				// Create a file to archive
				const sourceDir = join(tempDir, "source");
				yield* Effect.promise(() => mkdir(sourceDir, { recursive: true }));
				yield* Effect.promise(() => writeFile(join(sourceDir, "hello.txt"), "hello world"));
				...
				const result = yield* run(Effect.flatMap(ToolInstaller, (svc) => svc.extractTar(archivePath, destDir)));

				expect(result).toBe(destDir);
			}),
		);

		it.effect("creates a temp dir when dest is not provided", () =>
			Effect.gen(function* () {
				// Create a file to archive
				const sourceDir = join(tempDir, "source");
				yield* Effect.promise(() => mkdir(sourceDir, { recursive: true }));
				...
				const result = yield* run(Effect.flatMap(ToolInstaller, (svc) => svc.extractTar(archivePath)));
		...
		it.effect("supports custom tar flags", () =>
			Effect.gen(function* () {
				const sourceDir = join(tempDir, "source");
				yield* Effect.promise(() => mkdir(sourceDir, { recursive: true }));
				yield* Effect.promise(() => writeFile(join(sourceDir, "hello.txt"), "hello world"));
				...
				const destDir = join(tempDir, "extracted");
				const result = yield* run(
					Effect.flatMap(ToolInstaller, (svc) => svc.extractTar(archivePath, destDir, ["xzf"])),
				);
		...
		it.effect("uses default flags when an empty flags array is provided", () =>
			Effect.gen(function* () {
				const sourceDir = join(tempDir, "source");
				yield* Effect.promise(() => mkdir(sourceDir, { recursive: true }));
				yield* Effect.promise(() => writeFile(join(sourceDir, "hello.txt"), "hello world"));
		...
		it.effect("fails with ToolInstallerError for invalid archive", () =>
			Effect.gen(function* () {
				const badFile = join(tempDir, "bad.tar.gz");
				yield* Effect.promise(() => writeFile(badFile, "not a tar file"));
		...
		it.effect("fails with ToolInstallerError when spawn encounters a non-zero exit code", () =>
			Effect.gen(function* () {
				// Use a completely invalid archive to trigger a non-zero exit from tar
				const badFile = join(tempDir, "corrupt.tar.gz");
				yield* Effect.promise(() => writeFile(badFile, "this is definitely not a valid tar archive"));

				const destDir = join(tempDir, "nz-out");
				const error = yield* runFail(Effect.flatMap(ToolInstaller, (svc) => svc.extractTar(badFile, destDir)));
		...
		it.effect("fails with ToolInstallerError when the spawned process is killed by a signal", () =>
			Effect.gen(function* () {
				...
				yield* Effect.promise(() => mkdir(fakeBinDir, { recursive: true }));
				const fakeTar = join(fakeBinDir, "tar");
				yield* Effect.promise(() => writeFile(fakeTar, "#!/bin/sh\nkill -TERM $$\n"));
				const { chmod } = yield* Effect.promise(() => import("node:fs/promises"));
				yield* Effect.promise(() => chmod(fakeTar, 0o755));

				const originalPath = process.env.PATH;
				process.env.PATH = `${fakeBinDir}:${originalPath}`;

				const archivePath = join(tempDir, "fake.tar.gz");
				yield* Effect.promise(() => writeFile(archivePath, "fake"));
				const destDir = join(tempDir, "signal-out");

				try {
					const error = yield* runFail(Effect.flatMap(ToolInstaller, (svc) => svc.extractTar(archivePath, destDir)));
		...
		it.effect("fails with ToolInstallerError when the command cannot be found (spawn error event)", () =>
			Effect.gen(function* () {
		...
		it.effect("fails with ToolInstallerError when dest mkdir fails", () =>
			Effect.gen(function* () {
				const sourceDir = join(tempDir, "source");
				yield* Effect.promise(() => mkdir(sourceDir, { recursive: true }));
				yield* Effect.promise(() => writeFile(join(sourceDir, "hello.txt"), "hello world"));
				...
				yield* Effect.promise(() => writeFile(blockingFile, "i am a file, not a dir"));
				...
				const error = yield* runFail(
					Effect.flatMap(ToolInstaller, (svc) => svc.extractTar(archivePath, impossibleDest)),
				);
	...
	describe("extractZip", () => {
		it.effect("extracts a zip archive to a specified dest directory", () =>
			Effect.gen(function* () {
				const sourceDir = join(tempDir, "source");
				yield* Effect.promise(() => mkdir(sourceDir, { recursive: true }));
				yield* Effect.promise(() => writeFile(join(sourceDir, "hello.txt"), "hello world"));
				...
				const result = yield* run(Effect.flatMap(ToolInstaller, (svc) => svc.extractZip(archivePath, destDir)));

				expect(result).toBe(destDir);
			}),
		);

		it.effect("creates a temp dir when dest is not provided for zip", () =>
			Effect.gen(function* () {
				const sourceDir = join(tempDir, "source");
				yield* Effect.promise(() => mkdir(sourceDir, { recursive: true }));
				yield* Effect.promise(() => writeFile(join(sourceDir, "hello.txt"), "hello world"));
				...
				const result = yield* run(Effect.flatMap(ToolInstaller, (svc) => svc.extractZip(archivePath)));
		...
		it.effect("fails with ToolInstallerError for invalid zip archive", () =>
			Effect.gen(function* () {
				const badFile = join(tempDir, "bad.zip");
				yield* Effect.promise(() => writeFile(badFile, "not a zip file"));
		...
		it.effect(
			"uses PowerShell extraction on Windows platform",
			() =>
				Effect.gen(function* () {
					...
					try {
						const error = yield* runFail(Effect.flatMap(ToolInstaller, (svc) => svc.extractZip(archivePath, destDir)));
		...
		it.effect("fails with ToolInstallerError when zip dest mkdir fails", () =>
			Effect.gen(function* () {
				const sourceDir = join(tempDir, "source");
				yield* Effect.promise(() => mkdir(sourceDir, { recursive: true }));
				yield* Effect.promise(() => writeFile(join(sourceDir, "hello.txt"), "hello world"));
				...
				const error = yield* runFail(
					Effect.flatMap(ToolInstaller, (svc) => svc.extractZip(archivePath, impossibleDest)),
				);
	...
	describe("cacheDir", () => {
		it.effect("copies directory to tool cache path", () =>
			Effect.gen(function* () {
				const originalEnv = process.env.RUNNER_TOOL_CACHE;
				...
				try {
					const sourceDir = join(tempDir, "source");
					yield* Effect.promise(() => mkdir(sourceDir, { recursive: true }));
					yield* Effect.promise(() => writeFile(join(sourceDir, "bin.txt"), "binary content"));
					...
					const result = yield* run(Effect.flatMap(ToolInstaller, (svc) => svc.cacheDir(sourceDir, "mytool", "2.0.0")));
		...
		it.effect("fails with ToolInstallerError when source directory does not exist", () =>
			Effect.gen(function* () {
	...
	describe("cacheFile", () => {
		it.effect("copies file to tool cache path", () =>
			Effect.gen(function* () {
				const originalEnv = process.env.RUNNER_TOOL_CACHE;
				const cacheRoot = join(tempDir, "cache");
				process.env.RUNNER_TOOL_CACHE = cacheRoot;
				...
				try {
					...
					yield* Effect.promise(() => writeFile(sourceFile, "binary content"));
		...
		it.effect("fails with ToolInstallerError when source file does not exist", () =>
			Effect.gen(function* () {
	...
	describe("interruption", () => {
		it.live(
			"aborts the download request on interrupt",
			() =>
				Effect.gen(function* () {
					const { PassThrough } = yield* Effect.promise(() => import("node:stream"));
	...
	describe("retry predicate + backoff (TestClock)", () => {
		...
		it.effect("retries the download with exponential backoff then succeeds", () =>
			Effect.gen(function* () {
				let attempts = 0;
		...
		it.effect("gives up after the retry budget (recurs 2 → 3 total attempts)", () =>
			Effect.gen(function* () {
				let attempts = 0;
				const effect = Effect.suspend(() => {
					attempts++;
