...
const run = <A, E>(effect: Effect.Effect<A, E, ActionEnvironment>) => Effect.provide(effect, ActionEnvironmentLive);

const runExit = <A, E>(effect: Effect.Effect<A, E, ActionEnvironment>) =>
	Effect.exit(Effect.provide(effect, ActionEnvironmentLive));
...
describe("ActionEnvironmentLive", () => {
	describe("get", () => {
		it.effect("reads an environment variable", () =>
			Effect.gen(function* () {
				const result = yield* run(Effect.flatMap(ActionEnvironment, (svc) => svc.get("GITHUB_SHA")));
		...
		it.effect("fails on missing variable", () =>
			Effect.gen(function* () {
				...
				const exit = yield* runExit(Effect.flatMap(ActionEnvironment, (svc) => svc.get("MISSING_VAR")));
	...
	describe("getOptional", () => {
		it.effect("returns Some for set variable", () =>
			Effect.gen(function* () {
				const result = yield* run(Effect.flatMap(ActionEnvironment, (svc) => svc.getOptional("GITHUB_SHA")));
		...
		it.effect("returns None for missing variable", () =>
			Effect.gen(function* () {
				...
				const result = yield* run(Effect.flatMap(ActionEnvironment, (svc) => svc.getOptional("NOT_SET")));
	...
	describe("github", () => {
		it.effect("builds GitHub context from env vars", () =>
			Effect.gen(function* () {
				const result = yield* run(Effect.flatMap(ActionEnvironment, (svc) => svc.github));
	...
	describe("runner", () => {
		it.effect("builds runner context from env vars", () =>
			Effect.gen(function* () {
				const result = yield* run(Effect.flatMap(ActionEnvironment, (svc) => svc.runner));
		...
		it.effect("reads RUNNER_DEBUG=1 as true", () =>
			Effect.gen(function* () {
	...
	describe("isDebug", () => {
		it.effect("is true when RUNNER_DEBUG=1", () =>
			Effect.gen(function* () {
				process.env.RUNNER_DEBUG = "1";
				const result = yield* run(Effect.flatMap(ActionEnvironment, (svc) => svc.isDebug));
				expect(result).toBe(true);
			}),
		);

		it.effect("is false when RUNNER_DEBUG is unset", () =>
			Effect.gen(function* () {
		...
		it.effect("is false when RUNNER_DEBUG=0", () =>
			Effect.gen(function* () {
	...
	describe("payload / repo / issue", () => {
		...
		it.effect("payload parses GITHUB_EVENT_PATH JSON", () =>
			Effect.gen(function* () {
				...
				const result = yield* runFs(Effect.flatMap(ActionEnvironment, (svc) => svc.payload));
				expect(result.pull_request?.number).toBe(9);
			}),
		);

		it.effect("payload is empty when GITHUB_EVENT_PATH is unset", () =>
			Effect.gen(function* () {
		...
		it.effect("payload is empty (no throw) when the file is missing", () =>
			Effect.gen(function* () {
		...
		it.effect("payload fails ActionEnvironmentError on malformed JSON", () =>
			Effect.gen(function* () {
				const eventFile = join(tmp, "bad.json");
				writeFileSync(eventFile, "{ not json");
				overrideEnv({ GITHUB_EVENT_PATH: eventFile });
				const exit = yield* runFsExit(Effect.flatMap(ActionEnvironment, (svc) => svc.payload));
				expect(exit._tag).toBe("Failure");
		...
		it.effect("repo reads GITHUB_REPOSITORY first", () =>
			Effect.gen(function* () {
				overrideEnv({ GITHUB_REPOSITORY: "o/r", GITHUB_EVENT_PATH: undefined });
				const result = yield* runFs(Effect.flatMap(ActionEnvironment, (svc) => svc.repo));
				expect(result).toEqual({ owner: "o", repo: "r" });
			}),
		);

		it.effect("repo falls back to payload.repository", () =>
			Effect.gen(function* () {
				const eventFile = join(tmp, "event.json");
		...
		it.effect("repo fails when neither GITHUB_REPOSITORY nor payload.repository is available", () =>
			Effect.gen(function* () {
				overrideEnv({ GITHUB_REPOSITORY: undefined, GITHUB_EVENT_PATH: undefined });
				const exit = yield* runFsExit(Effect.flatMap(ActionEnvironment, (svc) => svc.repo));
				expect(exit._tag).toBe("Failure");
				if (exit._tag === "Failure") {
		...
		it.effect("issue resolves number from issue", () =>
			Effect.gen(function* () {
				const eventFile = join(tmp, "event.json");
				writeFileSync(eventFile, JSON.stringify({ issue: { number: 1 } }));
				overrideEnv({ GITHUB_REPOSITORY: "o/r", GITHUB_EVENT_PATH: eventFile });
				const result = yield* runFs(Effect.flatMap(ActionEnvironment, (svc) => svc.issue));
				expect(result).toEqual({ owner: "o", repo: "r", number: 1 });
			}),
		);

		it.effect("issue resolves number from pull_request", () =>
			Effect.gen(function* () {
				const eventFile = join(tmp, "event.json");
		...
		it.effect("issue resolves number from top-level payload.number", () =>
			Effect.gen(function* () {
				const eventFile = join(tmp, "event.json");
		...
		it.effect("issue fails when no number is present", () =>
			Effect.gen(function* () {
				overrideEnv({ GITHUB_REPOSITORY: "o/r", GITHUB_EVENT_PATH: undefined });
				const exit = yield* runFsExit(Effect.flatMap(ActionEnvironment, (svc) => svc.issue));
				expect(exit._tag).toBe("Failure");
			}),
		);
	});
