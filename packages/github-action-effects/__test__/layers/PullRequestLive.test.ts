...
import { PullRequestLive } from "../../src/layers/PullRequestLive.js";
...
const run = <A, E>(effect: Effect.Effect<A, E, PullRequest>) => Effect.provide(effect, testLayer);
...
describe("PullRequestLive", () => {
	describe("get", () => {
		it.effect("returns PullRequestInfo from pulls.get response", () =>
			Effect.gen(function* () {
				...
				const info = yield* run(Effect.flatMap(PullRequest, (svc) => svc.get(42)));
		...
		it.effect("PullRequestInfo carries mergedAt, body, and mergeCommitSha", () =>
			Effect.gen(function* () {
				mockGet.mockResolvedValue({
					data: {
						number: 7,
				...
				const info = yield* run(Effect.flatMap(PullRequest, (svc) => svc.get(7)));
		...
		it.effect("get maps the base SHA to baseSha", () =>
			Effect.gen(function* () {
				mockGet.mockResolvedValue({
					data: {
						number: 7,
	...
	describe("listFiles", () => {
		it.effect("listFiles returns the PR's changed files", () =>
			Effect.gen(function* () {
				...
				const files = yield* run(Effect.flatMap(PullRequest, (svc) => svc.listFiles(7)));
	...
	describe("listAssociatedWithCommit", () => {
		it.effect("listAssociatedWithCommit returns the associated PRs", () =>
			Effect.gen(function* () {
				...
				const prs = yield* run(Effect.flatMap(PullRequest, (svc) => svc.listAssociatedWithCommit("sha123")));
