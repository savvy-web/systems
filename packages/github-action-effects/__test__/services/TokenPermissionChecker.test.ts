...
import { TokenPermissionCheckerTest } from "../../src/layers/TokenPermissionCheckerTest.js";
...
describe("TokenPermissionChecker", () => {
	describe("check", () => {
		it.effect("returns satisfied=true when all permissions met", () =>
			Effect.gen(function* () {
				const state: ReturnType<typeof TokenPermissionCheckerTest.empty> = {
					grantedPermissions: { contents: "write", issues: "read" },
					checkCalls: [],
				};
				const result = yield* run(state, check({ contents: "write", issues: "read" }));
				expect(result.satisfied).toBe(true);
				expect(result.missing).toHaveLength(0);
			}),
		);

		it.effect("returns missing when permission insufficient", () =>
			Effect.gen(function* () {
				const state: ReturnType<typeof TokenPermissionCheckerTest.empty> = {
					grantedPermissions: { contents: "read" },
					checkCalls: [],
				};
				const result = yield* run(state, check({ contents: "write" }));
		...
		it.effect("returns extra when token has unrequested permissions", () =>
			Effect.gen(function* () {
				const state: ReturnType<typeof TokenPermissionCheckerTest.empty> = {
	...
	describe("assertSufficient", () => {
		it.effect("succeeds when satisfied", () =>
			Effect.gen(function* () {
				const state: ReturnType<typeof TokenPermissionCheckerTest.empty> = {
					grantedPermissions: { contents: "admin" },
					checkCalls: [],
				};
				const result = yield* run(state, assertSufficient({ contents: "write" }));
				expect(result.satisfied).toBe(true);
			}),
		);

		it.effect("fails when missing", () =>
			Effect.gen(function* () {
				const state: ReturnType<typeof TokenPermissionCheckerTest.empty> = {
					grantedPermissions: { contents: "read" },
					checkCalls: [],
				};
				const exit = yield* runExit(state, assertSufficient({ contents: "write" }));
	...
	describe("assertExact", () => {
		it.effect("succeeds when exact match", () =>
			Effect.gen(function* () {
				const state: ReturnType<typeof TokenPermissionCheckerTest.empty> = {
					grantedPermissions: { contents: "write" },
					checkCalls: [],
				};
				const result = yield* run(state, assertExact({ contents: "write" }));
				expect(result.satisfied).toBe(true);
				expect(result.extra).toHaveLength(0);
			}),
		);

		it.effect("fails when extra permissions present", () =>
			Effect.gen(function* () {
				const state: ReturnType<typeof TokenPermissionCheckerTest.empty> = {
					grantedPermissions: { contents: "write", issues: "read" },
					checkCalls: [],
				};
				const exit = yield* runExit(state, assertExact({ contents: "write" }));
				expect(Exit.isFailure(exit)).toBe(true);
				if (Exit.isFailure(exit)) {
					expect(String(exit.cause)).toContain("TokenPermissionError");
				}
	...
	describe("warnOverPermissioned", () => {
		it.effect("never fails and returns result with extras", () =>
			Effect.gen(function* () {
				const state: ReturnType<typeof TokenPermissionCheckerTest.empty> = {
					grantedPermissions: { contents: "write", issues: "admin", actions: "read" },
					checkCalls: [],
				};
				const result = yield* run(state, warnOverPermissioned({ contents: "write" }));
				expect(result.satisfied).toBe(true);
				expect(result.extra).toHaveLength(2);
			}),
		);
