...
describe("AttestLive.sbom — explicit BOM document", () => {
	it.effect("does NOT call Sbom.generate when bomDocument is supplied", () =>
		Effect.gen(function* () {
	...
	it.effect("calls Sbom.generate on the dependency-list path", () =>
		Effect.gen(function* () {
			const sbomState = makeSbomTestState();
			const layer = Layer.mergeAll(
				AttestLive,
	...
	it.effect("falls back to empty-deps when neither field is supplied", () =>
		Effect.gen(function* () {
			const sbomState = makeSbomTestState();
			const layer = Layer.mergeAll(
				AttestLive,
...
describe("AttestTest.sbom — captured calls expose bomDocument", () => {
	it.effect("records the bomDocument on the captured call", () =>
		Effect.gen(function* () {
