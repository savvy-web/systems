...
import { Sbom, SbomLive } from "../../src/testing.js";

const layer = Layer.merge(SbomLive, NodeFileSystem.layer);
const run = <A, E>(effect: Effect.Effect<A, E, Sbom | FileSystem.FileSystem>) => Effect.provide(effect, layer);
...
describe("Sbom service — step 5: CycloneDX BOM construction", () => {
	...
	describe("generate + serializeJson", () => {
		it.effect("produces a CycloneDX 1.5 BOM with the root component and dependencies", () =>
			Effect.gen(function* () {
		...
		it.effect("overrides a dependency version when an in-flight sibling is provided", () =>
			Effect.gen(function* () {
				const json = yield* run(
					Effect.gen(function* () {
						const sbom = yield* Sbom;
		...
		it.effect("adds in-flight packages that are not already in dependencies", () =>
			Effect.gen(function* () {
				const json = yield* run(
					Effect.gen(function* () {
						const sbom = yield* Sbom;
		...
		it.effect("carries NTIA supplier and author metadata when provided", () =>
			Effect.gen(function* () {
		...
		it.effect("accepts a supplier with only a name (no url or contact)", () =>
			Effect.gen(function* () {
				const json = yield* run(
					Effect.gen(function* () {
						const sbom = yield* Sbom;
		...
		it.effect("omits supplier and authors metadata when not provided", () =>
			Effect.gen(function* () {
				const json = yield* run(
					Effect.gen(function* () {
						const sbom = yield* Sbom;
		...
		it.effect("includes a metadata timestamp", () =>
			Effect.gen(function* () {
				const before = Date.now();
				const json = yield* run(
					Effect.gen(function* () {
	...
	describe("save", () => {
		it.effect("writes the serialized BOM to disk", () =>
			Effect.gen(function* () {
