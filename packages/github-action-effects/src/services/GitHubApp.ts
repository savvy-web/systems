...
export const InstallationToken = Schema.Struct({
	// `Schema.RedactedFromValue(Schema.String)` decodes a stored string into
	...
	permissions: Schema.Record(Schema.String, Schema.String).pipe(
		Schema.withDecodingDefaultType(Effect.sync(() => ({}))),
	),
}).annotate({ identifier: "InstallationToken" });
...
/**
 * Service shape for {@link GitHubApp}.
 *
 * @public
 */
 ...
 * @public
 */
export class GitHubApp extends Context.Service<GitHubApp, GitHubAppShape>()("github-action-effects/GitHubApp") {}
