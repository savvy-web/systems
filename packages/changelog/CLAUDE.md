...
- `@savvy-web/silk-effects`, `effect`, and `@effected/{commands,git,workspaces}` are declared runtime `dependencies`, externalized by the build and resolved by the consumer at import time (alongside `jju`/`semver`). Nothing is inlined.
