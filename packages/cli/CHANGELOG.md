...
- Usage errors now exit with code `64` (previously `1`), matching the `@effected/cli` convention for invalid invocations. [#698][#698]
...
- The `savvy` bin is unaffected for end users — it is the same command surface, now also shipped by `@savvy-web/silk` as a shim over `@savvy-web/cli/main`. The `./package.json` subpath is now exported. [#638][#638]
