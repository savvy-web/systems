...
- `savvy repos sync`/`add`/`pin` are enforcement points for the vendored-tree permissions boundary: silk-effects' `Repos.ReposLockdown` leaves `.repos/**` OS-level read-only after every mutation, so `ReposGroup` provides `Repos.ReposLockdown` alongside `ReposConfigStore`/`Git`, and `ReposLockdownError` belongs in the group's hand-written error annotation.
