...
- Top-level commands: `init`, `check`, `commit`, `changeset`, `lint`, `clean`, `repos` (the `repos` group — vendored `.repos/` reference repos — is an intentional expansion of the original six). `repos` subcommands: `status` (`--json`, `--drift` runs `Repos.ReposDrift.check`), `sync`, `add`, `pin`, `note`, `remove`, `rename`, `restore`.
- `savvy repos sync`/`add`/`pin`/`remove`/`rename`/`restore` are enforcement points for the vendored-tree permissions boundary: silk-effects' `Repos.ReposLockdown` leaves `.repos/**` OS-level read-only after every mutation, so `ReposGroup` provides `Repos.ReposLockdown` alongside `ReposConfigStore`/`Git`. Every handler `catchTag`s the full error union its `ReposManager` method can produce (`ReposLockdownError` included, for the six ops that unlock/re-lock the tree), so none of those propagate to the group's own hand-written error annotation — `reposCommand`'s error channel is `Repos.GitSubmoduleError` alone, the one channel `status`/`status --drift` leave uncaught.
