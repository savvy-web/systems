...
summary: "Identity envelope for a managed section type. Equal compares on `toolName` + `commentStyle`. Use to create a SectionBlock, or for a typed factory."
tier: packages
source: generated
...
Identity envelope for a managed section type. Equal compares on `toolName` + `commentStyle`. Use to create a [SectionBlock](silk://packages/silk-effects/api/class/sectionblock), or for a typed factory.
