...
Inputs passed to every [Attest](silk://packages/github-action-effects/api/class/attest) call.
...
Path → data captured by [Attest](silk://packages/github-action-effects/api/class/attest).
...
Pre-seeded attestation entries indexed by tarball sha256-hex. The [Attest](silk://packages/github-action-effects/api/class/attest) test implementation returns the matching entry list (filtered by `predicateType` when requested); an unseeded [subject](silk://packages/github-action-effects/api/function/subject) returns the empty array.
