...
summary: "Wraps `@effect/platform` `Command.Command` with instance method ergonomics. Use `yield* cmd.string()` instead of `yield* Command.string(cmd)`."
tier: packages
source: generated
...
Wraps `@effect/platform` `Command.Command` with instance method ergonomics. Use `yield* cmd.string()` instead of `yield* Command.string(cmd)`.
