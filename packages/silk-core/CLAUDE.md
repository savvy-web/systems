...
- **Kit ownership is unchanged; peer = nominal identity crosses the exported API.** `VersioningStrategy`/`TagStyle`/`PublishTarget` come from `@effected/workspaces` and `Section`/`SectionId`/`CommentStyle` from `@effected/templates` — both PEERS (`catalog:effected:peers`), not dependencies, because `SilkPublishConfig` extends workspaces' `PublishConfig` class and the `Savvy*Section` schemas build on templates' nominal classes, so silk-core and silk-effects (which peers on both) must resolve the same single copy or `instanceof`/schema identity breaks across the layer edge. The issue-reference grammar from `@effected/github-references` stays a plain dependency (`parseBareLines` returns an interface). `src/index.ts` re-exports nothing from the kit. See `okf/decisions/kit-effect-peers-via-catalog.md`.
...
What moved, the L1 boundary test, the shared scanner, why `@effected/workspaces` and `@effected/templates` are peers:
