...
- Ships `@savvy-web/bundler/ecma.json`, the shared TS base config, and `@savvy-web/bundler/env`, an ambient `.d.ts` export (sourced from `src/env.d.ts`) declaring the build-injected `process.env.__PACKAGE_VERSION__` — absent when running unbuilt source.
